...
/**
 * Created by yincheng on 2018/5/31/15:42.
 * github:luoyincheng
 */
public class AppHelper {
	...
	@Nullable
	public static Fragment getFragmentByTag(@NonNull FragmentManager fragmentManager,
	                                        @NonNull String tag) {
...
//         Toasty.success(App.getInstance(), context.getString(R.string.success_copied)).show();
			Toast.makeText(context, context.getString(R.string.success_copied), Toast.LENGTH_SHORT)
					.show();
		}
	}
...
//      String model = (!isEmulator()) ? DeviceNameGetter.getInstance().getDeviceName() :
// "Android Emulator";
//      StringBuilder builder = new StringBuilder()
//            .append("**FastHub Version: ").append(BuildConfig.VERSION_NAME).append(enterprise ?
// " Enterprise**" : "**").append("  \n")
//            .append(!isInstalledFromPlaySore(App.getInstance()) ? "**APK Source: Unknown**  \n"
// : "")
//            .append("**Android Version: ").append(String.valueOf(Build.VERSION.RELEASE)).append
// (" (SDK: ")
...
//   }
	...
	private static boolean isInstalledFromPlaySore(@NonNull Context context) {
		final String ipn = context.getPackageManager().getInstallerPackageName(BuildConfig
				.APPLICATION_ID);
...
//      try {
//         applicationInfo = context.getPackageManager().getApplicationInfo("com.google.android
// .gms", 0);
...
//   }
	...
	public static boolean isDeviceAnimationEnabled(@NonNull Context context) {
		float duration = Settings.Global.getFloat(context.getContentResolver(), Settings.Global
				.ANIMATOR_DURATION_SCALE, 1);
		float transition = Settings.Global.getFloat(context.getContentResolver(), Settings.Global
				.TRANSITION_ANIMATION_SCALE, 1);
		return (duration != 0 && transition != 0);
	}

	public static boolean isDataPlan() {
		final ConnectivityManager connectivityManager = (ConnectivityManager) App.getInstance()
				.getSystemService(Context.CONNECTIVITY_SERVICE);
		if (connectivityManager != null) {
			final android.net.NetworkInfo mobile = connectivityManager.getNetworkInfo
