 ...
 * <p>
 * When RecyclerView needs direct access to the view group children, it can transform unfiltered
...
class IChildHelper {
	...
	IChildHelper(Callback callback) {
		mCallback = callback;
		mBucket = new Bucket();
	 ...
	 *               IChildHelper offsets this index to actual ViewGroup index.
	 * @param hidden If set to true, this item will be invisible from regular methods.
	 */
	...
	View findHiddenNonRemovedView(int position) {
		...
		for (int i = 0; i < count; i++) {
			...
			IRecyclerView.ViewHolder holder = mCallback.getChildViewHolder(view);
	...
	/**
	 * Returns a child by ViewGroup offset. IChildHelper won't offset this index.
	 *
