...
import yincheng.tinytank.sourcecode.recyclerview.IRecyclerView.ItemDecoration;
import yincheng.tinytank.sourcecode.recyclerview.IRecyclerView.OnItemTouchListener;
import yincheng.tinytank.sourcecode.recyclerview.IRecyclerView.OnScrollListener;
...
class IFastScroller extends ItemDecoration implements OnItemTouchListener {
	...
	private final OnScrollListener mOnScrollListener = new OnScrollListener() {
		...
		public void onScrolled(IRecyclerView IRecyclerView, int dx, int dy) {
			updateScrollPosition(IRecyclerView.computeHorizontalScrollOffset(),
					IRecyclerView.computeVerticalScrollOffset());
		}
	};

	IFastScroller(IRecyclerView IRecyclerView, StateListDrawable verticalThumbDrawable,
		...
		attachToRecyclerView(IRecyclerView);
	}

	public void attachToRecyclerView(@Nullable IRecyclerView IRecyclerView) {
		if (mIRecyclerView == IRecyclerView) {
	...
	private void setupCallbacks() {
		mIRecyclerView.addItemDecoration(this);
		mIRecyclerView.addOnItemTouchListener(this);
		mIRecyclerView.addOnScrollListener(mOnScrollListener);
	}

	private void destroyCallbacks() {
		mIRecyclerView.removeItemDecoration(this);
		mIRecyclerView.removeOnItemTouchListener(this);
		mIRecyclerView.removeOnScrollListener(mOnScrollListener);
		cancelHide();
	}
	...
	private boolean isLayoutRTL() {
		return ViewCompat.getLayoutDirection(mIRecyclerView) == ViewCompat.LAYOUT_DIRECTION_RTL;
	}
	...
	private void cancelHide() {
		mIRecyclerView.removeCallbacks(mHideRunnable);
	}

	private void resetHideDelay(int delay) {
		cancelHide();
		mIRecyclerView.postDelayed(mHideRunnable, delay);
	}

	@Override
	public void onDrawOver(Canvas canvas, IRecyclerView parent, IRecyclerView.State state) {
		if (mRecyclerViewWidth != mIRecyclerView.getWidth()
				|| mRecyclerViewHeight != mIRecyclerView.getHeight()) {
			mRecyclerViewWidth = mIRecyclerView.getWidth();
			mRecyclerViewHeight = mIRecyclerView.getHeight();
	...
	void updateScrollPosition(int offsetX, int offsetY) {
		int verticalContentLength = mIRecyclerView.computeVerticalScrollRange();
		int verticalVisibleLength = mRecyclerViewHeight;
		mNeedVerticalScrollbar = verticalContentLength - verticalVisibleLength > 0
				&& mRecyclerViewHeight >= mScrollbarMinimumRange;

		int horizontalContentLength = mIRecyclerView.computeHorizontalScrollRange();
	...
	@Override
	public boolean onInterceptTouchEvent(IRecyclerView IRecyclerView, MotionEvent ev) {
	...
	@Override
	public void onTouchEvent(IRecyclerView IRecyclerView, MotionEvent me) {
		if (mState == STATE_HIDDEN) {
			return;
		}
	...
	private void verticalScrollTo(float y) {
		...
		int scrollingBy = scrollTo(mVerticalDragY, y, scrollbarRange,
				mIRecyclerView.computeVerticalScrollRange(),
				mIRecyclerView.computeVerticalScrollOffset(), mRecyclerViewHeight);
		if (scrollingBy != 0) {
			mIRecyclerView.scrollBy(0, scrollingBy);
		}
		mVerticalDragY = y;
	}
	...
	private void horizontalScrollTo(float x) {
		...
		int scrollingBy = scrollTo(mHorizontalDragX, x, scrollbarRange,
				mIRecyclerView.computeHorizontalScrollRange(),
				mIRecyclerView.computeHorizontalScrollOffset(), mRecyclerViewWidth);
		if (scrollingBy != 0) {
			mIRecyclerView.scrollBy(scrollingBy, 0);
		}
