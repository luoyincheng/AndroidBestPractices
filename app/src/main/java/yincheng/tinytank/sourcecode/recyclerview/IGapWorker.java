...
final class IGapWorker implements Runnable {
	...
	public void add(IRecyclerView IRecyclerView) {
		if (IRecyclerView.DEBUG && mIRecyclerViews.contains(IRecyclerView)) {
			throw new IllegalStateException("RecyclerView already present in worker list!");
		}
		mIRecyclerViews.add(IRecyclerView);
	}

	public void remove(IRecyclerView IRecyclerView) {
		boolean removeSuccess = mIRecyclerViews.remove(IRecyclerView);
		if (IRecyclerView.DEBUG && !removeSuccess) {
	 ...
	 */
	void postFromTraversal(IRecyclerView IRecyclerView, int prefetchDx, int prefetchDy) {
		if (IRecyclerView.isAttachedToWindow()) {
			if (IRecyclerView.DEBUG && !mIRecyclerViews.contains(IRecyclerView)) {
				throw new IllegalStateException("attempting to post unregistered view!");
			}
			if (mPostTimeNs == 0) {
				mPostTimeNs = IRecyclerView.getNanoTime();
				IRecyclerView.post(this);
			}
		}
	...
	private void buildTaskList() {
		...
		final int viewCount = mIRecyclerViews.size();
		int totalTaskCount = 0;
		for (int i = 0; i < viewCount; i++) {
			IRecyclerView view = mIRecyclerViews.get(i);
	...
	private IRecyclerView.ViewHolder prefetchPositionWithDeadline(IRecyclerView view,
		...
		IRecyclerView.Recycler recycler = view.mRecycler;
		IRecyclerView.ViewHolder holder;
		try {
			...
			if (holder != null) {
				if (holder.isBound() && !holder.isInvalid()) {
					...
					// type, during this IGapWorker prefetch pass).
	...
	private void prefetchInnerRecyclerViewWithDeadline(@Nullable IRecyclerView innerView,
	                                                   long deadlineNs) {
		if (innerView == null) {
			return;
		}
		...
		if (innerView.mDataSetHasChangedAfterLayout
				&& innerView.mIChildHelper.getUnfilteredChildCount() != 0) {
		...
		if (innerPrefetchRegistry.mCount != 0) {
			try {
				TraceCompat.beginSection(IRecyclerView.TRACE_NESTED_PREFETCH_TAG);
	...
	private void flushTaskWithDeadline(Task task, long deadlineNs) {
		long taskDeadlineNs = task.immediate ? IRecyclerView.FOREVER_NS : deadlineNs;
		IRecyclerView.ViewHolder holder = prefetchPositionWithDeadline(task.view,
	...
	public void run() {
		try {
			TraceCompat.beginSection(IRecyclerView.TRACE_PREFETCH_TAG);

			if (mIRecyclerViews.isEmpty()) {
				// abort - no work to do
				return;
			}
			...
			final int size = mIRecyclerViews.size();
