 ...
 * animations.
 * <p>
 * IAdapterHelper creates an UpdateOp for each adapter data change then pre-processes them. During
 * pre processing, IAdapterHelper finds out which UpdateOps can be deferred to second layout pass
 * and which cannot. For the UpdateOps that cannot be deferred, IAdapterHelper will change them
 ...
 */
public class IAdapterHelper implements IOpReOrderer.Callback {
	...
	final IOpReOrderer mIOpReOrderer;

	private int mExistingUpdateTypes = 0;

	IAdapterHelper(Callback callback) {
		this(callback, false);
	}

	IAdapterHelper(Callback callback, boolean disableRecycler) {
		mCallback = callback;
		mDisableRecycler = disableRecycler;
		mIOpReOrderer = new IOpReOrderer(this);
	}

	IAdapterHelper addUpdateOp(UpdateOp... ops) {
	...
	void preProcess() {
		mIOpReOrderer.reorderOps(mPendingUpdates);
	...
	private void applyRemove(UpdateOp op) {
		...
		for (int position = op.positionStart; position < tmpEnd; position++) {
			...
			IRecyclerView.ViewHolder vh = mCallback.findViewHolder(position);
			if (vh != null || canFindInPreLayout(position)) {
				...
				// have any effect in pre-layout since their sell ops are already deferred to
				// post-layout pass.
	...
	private void dispatchAndUpdateViewHolders(UpdateOp op) {
		...
		if (op.cmd == UpdateOp.ADD || op.cmd == UpdateOp.MOVE) {
			throw new IllegalArgumentException("should not dispatch sell or move for pre layout");
		}
		if (DEBUG) {
		...
		// handleBusiness each pos 1 by 1 to ensure continuity. If it breaks, dispatch partial
	...
	void consumeUpdatesInOnePass() {
		// we still consume postponed updates (if there is) in case there was a pre-process transform
	...
	/**
	 * Contract between IAdapterHelper and RecyclerView.
	 */
	interface Callback {

		IRecyclerView.ViewHolder findViewHolder(int position);
