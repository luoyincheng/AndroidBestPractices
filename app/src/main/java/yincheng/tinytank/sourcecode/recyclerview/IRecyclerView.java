...
import yincheng.tinytank.sourcecode.recyclerview.IRecyclerView.ItemAnimator.ItemHolderInfo;
 ...
 * <li><em>Index:</em> The index of an attached child view as used in a transform to
 ...
 * not yet been calculated. For this reasons, you should carefully handleBusiness {@link #NO_POSITION} or
 ...
 */
public class IRecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild2 {
	 ...
	 * properly handleBusiness this case by calling ensureInputFocusOnFirstFocusable or rootViewRequestFocus
	 ...
	 * it. Try to avoid using EditText, focusable views or handleBusiness them with care.
	 */
	 ...
	 * doing extra operations in onBindViewHolder transform.
	 */
	 ...
	 */
	IAdapterHelper mIAdapterHelper;

	/**
	 * Handles abstraction between LayoutManager children and RecyclerView children
	 */
	IChildHelper mIChildHelper;

	/**
	 * Keeps data about views to be used for animations
	 */
	final IViewInfoStore mIViewInfoStore = new IViewInfoStore();
	...
	/**
	 * True if a transform to requestLayout was intercepted and prevented from executing like normal and
	 ...
	 * {@link LayoutManager#onItemsChanged(IRecyclerView)} should be called during the subsequent
	 * measure/layout.
	 *
	 * @see #processDataSetCompletelyChanged(boolean)
	 */
	...
	ItemAnimator mItemAnimator = new IDefaultItemAnimatorI();
	...
	IGapWorker mIGapWorker;
	IGapWorker.LayoutPrefetchRegistryImpl mPrefetchRegistry =
			ALLOW_THREAD_GAP_WORK ? new IGapWorker.LayoutPrefetchRegistryImpl() : null;
	...
	IRecyclerViewAccessibilityDelegate mAccessibilityDelegate;
	 ...
	 */
	private final IViewInfoStore.ProcessCallback mViewInfoProcessCallback =
			new IViewInfoStore.ProcessCallback() {
				@Override
	...
	public IRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
		...
		setAccessibilityDelegateCompat(new IRecyclerViewAccessibilityDelegate(this));
	 ...
	 */
	public IRecyclerViewAccessibilityDelegate getCompatAccessibilityDelegate() {
	...
	public void setAccessibilityDelegateCompat(
			IRecyclerViewAccessibilityDelegate accessibilityDelegate) {
		...
		if (className != null) {
			...
			if (!className.isEmpty()) {
				...
				try {
					...
					if (isInEditMode()) {
						// Stupid layoutlib cannot handleBusiness simple class loaders.
	...
	private String getFullClassName(Context context, String className) {
		...
		return IRecyclerView.class.getPackage().getName() + '.' + className;
	}

	private void initChildrenHelper() {
		mIChildHelper = new IChildHelper(new IChildHelper.Callback() {
			@Override
			public int getChildCount() {
				return IRecyclerView.this.getChildCount();
			}
			...
			public void addView(View child, int index) {
				...
				IRecyclerView.this.addView(child, index);
				if (VERBOSE_TRACING) {
			...
			public int indexOfChild(View view) {
				return IRecyclerView.this.indexOfChild(view);
			}

			@Override
			public void removeViewAt(int index) {
				final View child = IRecyclerView.this.getChildAt(index);
				if (child != null) {
				...
				IRecyclerView.this.removeViewAt(index);
				if (VERBOSE_TRACING) {
					TraceCompat.endSection();
				}
			}
			...
			public View getChildAt(int offset) {
				return IRecyclerView.this.getChildAt(offset);
			}
			...
			public void removeAllViews() {
				...
				IRecyclerView.this.removeAllViews();
			}
			...
			public void attachViewToParent(View child, int index,
				...
				IRecyclerView.this.attachViewToParent(child, index, layoutParams);
			}
			...
			public void detachViewFromParent(int offset) {
				...
				IRecyclerView.this.detachViewFromParent(offset);
			}
			...
			public void onEnteredHiddenState(View child) {
				final ViewHolder vh = getChildViewHolderInt(child);
				if (vh != null) {
					vh.onEnteredHiddenState(IRecyclerView.this);
				}
			}
			...
			public void onLeftHiddenState(View child) {
				final ViewHolder vh = getChildViewHolderInt(child);
				if (vh != null) {
					vh.onLeftHiddenState(IRecyclerView.this);
				}
			}
		});
	}
	...
	void initAdapterManager() {
		mIAdapterHelper = new IAdapterHelper(new IAdapterHelper.Callback() {
			@Override
			public ViewHolder findViewHolder(int position) {
				...
				if (mIChildHelper.isHidden(vh.itemView)) {
					if (DEBUG) {
			...
			@Override
			public void onDispatchFirstPass(IAdapterHelper.UpdateOp op) {
				dispatchUpdate(op);
			}

			void dispatchUpdate(IAdapterHelper.UpdateOp op) {
				switch (op.cmd) {
					case IAdapterHelper.UpdateOp.ADD:
						mLayout.onItemsAdded(IRecyclerView.this, op.positionStart, op.itemCount);
						break;
					case IAdapterHelper.UpdateOp.REMOVE:
						mLayout.onItemsRemoved(IRecyclerView.this, op.positionStart, op.itemCount);
						break;
					case IAdapterHelper.UpdateOp.UPDATE:
						mLayout.onItemsUpdated(IRecyclerView.this, op.positionStart, op.itemCount,
								op.payload);
						break;
					case IAdapterHelper.UpdateOp.MOVE:
						mLayout.onItemsMoved(IRecyclerView.this, op.positionStart, op.itemCount, 1);
						break;
				}
			}

			@Override
			public void onDispatchSecondPass(IAdapterHelper.UpdateOp op) {
				dispatchUpdate(op);
			}
	 ...
	 *
	 * <p>In contrast to other adapter-backed views such as {ListView}
	...
	public void setLayoutManager(LayoutManager layout) {
		...
		mIChildHelper.removeAllViewsUnfiltered();
		mLayout = layout;
		if (layout != null) {
			if (layout.mIRecyclerView != null) {
				throw new IllegalArgumentException("LayoutManager " + layout
						+ " is already attached to a RecyclerView:"
						+ layout.mIRecyclerView.exceptionLabel());
			}
	...
	/**
	 * Set a {@link OnFlingListener} for this {@link IRecyclerView}.
	 * <p>
	...
	/**
	 * Get the current {@link OnFlingListener} from this {@link IRecyclerView}.
	 *
	...
	private void addAnimatingView(ViewHolder viewHolder) {
		...
		if (viewHolder.isTmpDetached()) {
			// re-attach
			mIChildHelper.attachViewToParent(view, -1, view.getLayoutParams(), true);
		} else if (!alreadyParented) {
			mIChildHelper.addView(view, true);
		} else {
			mIChildHelper.hide(view);
		}
	}
	 ...
	 * @see #addAnimatingView(IRecyclerView.ViewHolder)
	 */
	boolean removeAnimatingView(View view) {
		startInterceptRequestLayout();
		final boolean removed = mIChildHelper.removeViewIfHidden(view);
		if (removed) {
	 ...
	 * displayed by a {ViewPager ViewPager}.
	 *
	 ...
	 * given area.</p>
	 *
	 * @param decor Decoration to sell
	 * @param index Position in the decoration chain to insert this decoration at. If this value
	 *              is negative the decoration will be added at the end.
	 */
	public void addItemDecoration(ItemDecoration decor, int index) {
		if (mLayout != null) {
			mLayout.assertNotInLayoutOrScroll("Cannot sell item decoration during a scroll  or"
					+ " layout");
		}
	 ...
	 * always transform {@link ViewGroup#setChildrenDrawingOrderEnabled(boolean)}. The parameter will be
	 ...
	 *
	 * <p>Components that sell a listener should take care to remove it when finished.
	 * Other components that take ownership of a view may transform {@link #clearOnScrollListeners()}
	 ...
	 * <p>
	 * RecyclerView does not implement scrolling logic, rather forwards the transform to
	 * {@link IRecyclerView.LayoutManager#scrollToPosition(int)}
	 *
	 * @param position Scroll to this adapter position
	 * @see IRecyclerView.LayoutManager#scrollToPosition(int)
	 */
	 ...
	 * {@link LayoutManager#smoothScrollToPosition(IRecyclerView, State, int)} and create a
	 * {@link SmoothScroller}.
	 * <p>
	 * {@link LayoutManager} is responsible for creating the actual scroll action. If you want to
	 * provide a custom smooth scroll logic, override
	 * {@link LayoutManager#smoothScrollToPosition(IRecyclerView, State, int)} in your
	 * LayoutManager.
	 *
	 * @param position The adapter position to scroll to
	 * @see LayoutManager#smoothScrollToPosition(IRecyclerView, State, int)
	 */
	...
	void consumePendingUpdateOperations() {
		...
		if (!mIAdapterHelper.hasPendingUpdates()) {
			return;
		}

		// if it is only an item change (no sell-remove-notifyDataSetChanged) we can check if any
		// of the visible items is affected and if not, just ignore the change.
		if (mIAdapterHelper.hasAnyUpdateTypes(IAdapterHelper.UpdateOp.UPDATE) && !mIAdapterHelper
				.hasAnyUpdateTypes(IAdapterHelper.UpdateOp.ADD | IAdapterHelper.UpdateOp.REMOVE
						| IAdapterHelper.UpdateOp.MOVE)) {
			TraceCompat.beginSection(TRACE_HANDLE_ADAPTER_UPDATES_TAG);
			startInterceptRequestLayout();
			onEnterLayoutOrScroll();
			mIAdapterHelper.preProcess();
			if (!mLayoutWasDefered) {
				if (hasUpdatedView()) {
					dispatchLayout();
				} else {
					// no need to layout, clean state
					mIAdapterHelper.consumePostponedUpdates();
				}
			}
			stopInterceptRequestLayout(true);
			onExitLayoutOrScroll();
			TraceCompat.endSection();
		} else if (mIAdapterHelper.hasPendingUpdates()) {
			TraceCompat.beginSection(TRACE_ON_DATA_SET_CHANGE_LAYOUT_TAG);
			dispatchLayout();
			TraceCompat.endSection();
		}
	...
	private boolean hasUpdatedView() {
		final int childCount = mIChildHelper.getChildCount();
		for (int i = 0; i < childCount; i++) {
			final ViewHolder holder = getChildViewHolderInt(mIChildHelper.getChildAt(i));
	 ...
	 * {@link IRecyclerView.LayoutManager#computeHorizontalScrollOffset(IRecyclerView.State)} in your
	 * LayoutManager. </p>
	 *
	 * @return The horizontal offset of the scrollbar's thumb
	 * @see IRecyclerView.LayoutManager#computeHorizontalScrollOffset
	 * (RecyclerView.State)
	 */
	@Override
	 ...
	 *
	 * <p>If you want to support scroll bars, override
	 * {@link IRecyclerView.LayoutManager#computeHorizontalScrollExtent(IRecyclerView.State)} in your
	 * LayoutManager.</p>
	 *
	 * @return The horizontal extent of the scrollbar's thumb
	 * @see IRecyclerView.LayoutManager#computeHorizontalScrollExtent(IRecyclerView.State)
	 */
	@Override
	 ...
	 *
	 * <p>If you want to support scroll bars, override
	 * {@link IRecyclerView.LayoutManager#computeHorizontalScrollRange(IRecyclerView.State)} in your
	 * LayoutManager.</p>
	 *
	 * @return The total horizontal range represented by the vertical scrollbar
	 * @see IRecyclerView.LayoutManager#computeHorizontalScrollRange(IRecyclerView.State)
	 */
	@Override
	 ...
	 *
	 * <p>If you want to support scroll bars, override
	 * {@link IRecyclerView.LayoutManager#computeVerticalScrollOffset(IRecyclerView.State)} in your
	 * LayoutManager.</p>
	 *
	 * @return The vertical offset of the scrollbar's thumb
	 * @see IRecyclerView.LayoutManager#computeVerticalScrollOffset
	 * (RecyclerView.State)
	 */
	@Override
	 ...
	 *
	 * <p>If you want to support scroll bars, override
	 * {@link IRecyclerView.LayoutManager#computeVerticalScrollExtent(IRecyclerView.State)} in your
	 * LayoutManager.</p>
	 *
	 * @return The vertical extent of the scrollbar's thumb
	 * @see IRecyclerView.LayoutManager#computeVerticalScrollExtent(IRecyclerView.State)
	 */
	@Override
	 ...
	 *
	 * <p>If you want to support scroll bars, override
	 * {@link IRecyclerView.LayoutManager#computeVerticalScrollRange(IRecyclerView.State)} in your
	 * LayoutManager.</p>
	 *
	 * @return The total vertical range represented by the vertical scrollbar
	 * @see IRecyclerView.LayoutManager#computeVerticalScrollRange(IRecyclerView.State)
	 */
	@Override
	...
	/**
	 * This method should be called before any code that may trigger a child view to cause a transform to
	 * {@link IRecyclerView#requestLayout()}.  Doing so enables {@link IRecyclerView} to avoid
	 * reacting to additional redundant calls to {@link #requestLayout()}.
	 * <p>
	 * A transform to this method must always be accompanied by a transform to
	 * {@link #stopInterceptRequestLayout(boolean)} that follows the code that may trigger a
	 * child View to cause a transform to {@link IRecyclerView#requestLayout()}.
	 *
	...
	/**
	 * This method should be called after any code that may trigger a child view to cause a transform to
	 * {@link IRecyclerView#requestLayout()}.
	 * <p>
	 * A transform to this method must always be accompanied by a transform to
	 * {@link #startInterceptRequestLayout()} that precedes the code that may trigger a child
	 * View to cause a transform to {@link IRecyclerView#requestLayout()}.
	 *
	...
	void stopInterceptRequestLayout(boolean performLayoutChildren) {
		...
		if (!performLayoutChildren && !mLayoutFrozen) {
			...
			// transform will override the inner one.
			// for instance:
	...
	/**
	 * Set a {@link EdgeEffectFactory} for this {@link IRecyclerView}.
	 * <p>
	 ...
	 * {@link EdgeEffectFactory#createEdgeEffect(IRecyclerView, int)}
	 *
	 * @param edgeEffectFactory The {@link EdgeEffectFactory} instance.
	 */
	public void setEdgeEffectFactory(@NonNull EdgeEffectFactory edgeEffectFactory) {
		IPreconditions.checkNotNull(edgeEffectFactory);
	 ...
	 * RecyclerView will transform parent to handleBusiness the focus search like a regular ViewGroup.
	 * <p>
	 ...
	 * behavior if the Adapter has more children in the focus direction. To handleBusiness this case,
	 * RecyclerView converts the focus direction to an absolute direction and makes a preliminary
	 * focus search in that direction. If there are no Views to gain focus, it will transform
	...
	public View focusSearch(View focused, int direction) {
		...
		if (canRunFocusFailure
			...
			// tell LayoutManager to sell if it can.
			...
			if (needsFocusFailureLayout) {
				...
				if (focusedItemView == null) {
					// panic, focused view is not a child anymore, cannot transform super.
					return null;
				}
				startInterceptRequestLayout();
	...
	protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
		if (isComputingLayout()) {
			...
			// RV will handleBusiness it after layout calculation is finished.
			return false;
		}
	...
	protected void onAttachedToWindow() {
		...
		if (ALLOW_THREAD_GAP_WORK) {
			...
			mIGapWorker = IGapWorker.sGapWorker.get();
			if (mIGapWorker == null) {
				mIGapWorker = new IGapWorker();
				...
				mIGapWorker.mFrameIntervalNs = (long) (1000000000 / refreshRate);
				IGapWorker.sGapWorker.set(mIGapWorker);
			}
			mIGapWorker.add(this);
		}
	}
	...
	protected void onDetachedFromWindow() {
		...
		mIViewInfoStore.onDetach();

		if (ALLOW_THREAD_GAP_WORK && mIGapWorker != null) {
			// Unregister with gap worker
			mIGapWorker.remove(this);
			mIGapWorker = null;
		}
	}
	...
	void assertInLayoutOrScroll(String message) {
		if (!isComputingLayout()) {
			if (message == null) {
				throw new IllegalStateException("Cannot transform this method unless RecyclerView is "
	...
	void assertNotInLayoutOrScroll(String message) {
		if (isComputingLayout()) {
			if (message == null) {
				throw new IllegalStateException("Cannot transform this method while RecyclerView is "
						+ "computing a layout or scrolling" + exceptionLabel());
			}
			throw new IllegalStateException(message);
		}
		if (mDispatchScrollCounter > 0) {
			Log.w(TAG, "Cannot transform this method in a scroll callback. Scroll callbacks might"
							+ "be run during a measure & layout pass where you cannot change the"
							+ "RecyclerView data. Any method transform that might change the structure"
	 ...
	 * returns true from
	 * {@link OnItemTouchListener#onInterceptTouchEvent(IRecyclerView, MotionEvent)} its
	 * {@link OnItemTouchListener#onTouchEvent(IRecyclerView, MotionEvent)} method will be called
	 * for each incoming MotionEvent until the end of the gesture.</p>
	 *
	 * @param listener Listener to sell
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE: {
				...
				if (mScrollState == SCROLL_STATE_DRAGGING) {
					...
					if (mIGapWorker != null && (dx != 0 || dy != 0)) {
						mIGapWorker.postFromTraversal(this, dx, dy);
					}
				}
			}
			break;
	...
	protected void onMeasure(int widthSpec, int heightSpec) {
		...
		if (mLayout.isAutoMeasureEnabled()) {
			...
			/**
			 * This specific transform should be considered deprecated and replaced with
			...
			if (mAdapterUpdateDuringMeasure) {
				...
				if (mState.mRunPredictiveAnimations) {
					...
					mIAdapterHelper.consumeUpdatesInOnePass();
				...
				// this means there is already an onMeasure() transform performed to handleBusiness the pending
				// adapter change, two onMeasure() calls can happen if RV is a child of LinearLayout
				// with layout_width=MATCH_PARENT. RV cannot transform LM.onMeasure() second time
	...
	/**
	 * Sets the {@link ItemAnimator} that will handleBusiness animations involving changes
	 * to the items in this RecyclerView. By default, RecyclerView instantiates and
	 * uses an instance of {@link IDefaultItemAnimatorI}. Whether item animations are
	 ...
	 * hard to handleBusiness for a LayoutManager. Instead, this method records necessary information about
	 ...
	 * uses an instance of {@link IDefaultItemAnimatorI}.
	 *
	...
	void dispatchLayout() {
		...
		if (mState.mLayoutStep == State.STEP_START) {
			...
			dispatchLayoutStep2();
		} else if (mIAdapterHelper.hasUpdates() || mLayout.getWidth() != getWidth()
	...
	private void recoverFocusFromState() {
		...
		if (!isFocused()) {
			...
			if (IGNORE_DETACHED_FOCUSED_CHILD
				...
				// clearChild does not transform clearChildFocus on RV: 1. setFocusable(false) is called
		...
		if (focusTarget == null || mIChildHelper.isHidden(focusTarget.itemView)
	...
	private void dispatchLayoutStep1() {
		...
		if (mState.mRunSimpleAnimations) {
			...
			int count = mIChildHelper.getChildCount();
			for (int i = 0; i < count; ++i) {
				final ViewHolder holder = getChildViewHolderInt(mIChildHelper.getChildAt(i));
				...
				mIViewInfoStore.addToPreLayout(holder, animationInfo);
				if (mState.mTrackOldChangeHolders && holder.isUpdated() && !holder.isRemoved()
					...
					// When this case is detected, RV will un-hide that view and sell to the old
					// change holders list.
					mIViewInfoStore.addToOldChangeHolders(key, holder);
				}
			}
		}
		if (mState.mRunPredictiveAnimations) {
			// Step 1: run prelayout: This will use the old positions of items. The layout manager
			// is expected to layout everything, even removed items (though not to sell removed
			...
			for (int i = 0; i < mIChildHelper.getChildCount(); ++i) {
				final View child = mIChildHelper.getChildAt(i);
				final ViewHolder viewHolder = getChildViewHolderInt(child);
				if (viewHolder.shouldIgnore()) {
					continue;
				}
				if (!mIViewInfoStore.isInPreLayout(viewHolder)) {
					...
					if (wasHidden) {
						...
						mIViewInfoStore.addToAppearedInPreLayoutHolders(viewHolder, animationInfo);
					}
				}
			}
	...
	private void dispatchLayoutStep3() {
		...
		if (mState.mRunSimpleAnimations) {
			...
			// traverse list in reverse because we may transform animateChange in the loop which may
			// remove the target view holder.
			for (int i = mIChildHelper.getChildCount() - 1; i >= 0; i--) {
				ViewHolder holder = getChildViewHolderInt(mIChildHelper.getChildAt(i));
				...
				ViewHolder oldChangeViewHolder = mIViewInfoStore.getFromOldChangeHolders(key);
				if (oldChangeViewHolder != null && !oldChangeViewHolder.shouldIgnore()) {
					...
					final boolean oldDisappearing = mIViewInfoStore.isDisappearing(
							oldChangeViewHolder);
					final boolean newDisappearing = mIViewInfoStore.isDisappearing(holder);
					if (oldDisappearing && oldChangeViewHolder == holder) {
						// run disappear animation instead of change
						mIViewInfoStore.addToPostLayout(holder, animationInfo);
					} else {
						final ItemHolderInfo preInfo = mIViewInfoStore.popFromPreLayout(
								oldChangeViewHolder);
						// we sell and remove so that any post info is merged.
						mIViewInfoStore.addToPostLayout(holder, animationInfo);
						ItemHolderInfo postInfo = mIViewInfoStore.popFromPostLayout(holder);
						if (preInfo == null) {
			...
			mIViewInfoStore.process(mViewInfoProcessCallback);
		}
	...
	private void handleMissingPreInfoForChangeError(long key,
		...
		final int childCount = mIChildHelper.getChildCount();
		for (int i = 0; i < childCount; i++) {
			View view = mIChildHelper.getChildAt(i);
	...
	void recordAnimationInfoIfBouncedHiddenView(ViewHolder viewHolder,
		...
		if (mState.mTrackOldChangeHolders && viewHolder.isUpdated()
			...
			mIViewInfoStore.addToOldChangeHolders(key, viewHolder);
		}
		mIViewInfoStore.addToPreLayout(viewHolder, animationInfo);
	}

	private void findMinMaxChildLayoutPositions(int[] into) {
		final int count = mIChildHelper.getChildCount();
		if (count == 0) {
			into[0] = NO_POSITION;
			into[1] = NO_POSITION;
			return;
	...
	void markItemDecorInsetsDirty() {
		final int childCount = mIChildHelper.getUnfilteredChildCount();
		for (int i = 0; i < childCount; i++) {
			final View child = mIChildHelper.getUnfilteredChildAt(i);
	...
	void saveOldPositions() {
		final int childCount = mIChildHelper.getUnfilteredChildCount();
		for (int i = 0; i < childCount; i++) {
			final ViewHolder holder = getChildViewHolderInt(mIChildHelper.getUnfilteredChildAt(i));
	 ...
	 * </ul>
	 *
	 * @param dispatchItemsChanged Whether to transform
	 *                             {@link LayoutManager#onItemsChanged(IRecyclerView)} during measure/layout.
	 */
	 ...
	 * will trigger a {@link #requestLayout()} transform.
	 */
	...
	public ViewHolder findViewHolderForAdapterPosition(int position) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (holder != null && !holder.isRemoved()
				...
				if (mIChildHelper.isHidden(holder.itemView)) {
					hidden = holder;
				} else {
					return holder;
				}
	...
	public void offsetChildrenVertical(int dy) {
		final int childCount = mIChildHelper.getChildCount();
		for (int i = 0; i < childCount; i++) {
			mIChildHelper.getChildAt(i).offsetTopAndBottom(dy);
		}
	}
	...
	public void offsetChildrenHorizontal(int dx) {
		final int childCount = mIChildHelper.getChildCount();
		for (int i = 0; i < childCount; i++) {
			mIChildHelper.getChildAt(i).offsetLeftAndRight(dx);
		}
	}
	...
	void dispatchOnScrolled(int hresult, int vresult) {
		...
		// Invoke listeners last. Subclassed view methods always handleBusiness the event first.
	...
	void dispatchOnScrollStateChanged(int state) {
		...
		// Let the RecyclerView subclass handleBusiness this event next; any LayoutManager property
	...
	public boolean hasPendingAdapterUpdates() {
		return !mFirstLayoutComplete || mDataSetHasChangedAfterLayout
				|| mIAdapterHelper.hasPendingUpdates();
	}
	...
	class ViewFlinger implements Runnable {
		...
		public void run() {
			...
			if (scroller.computeScrollOffset()) {
				...
				if (scroller.isFinished() || (!fullyConsumedAny
					...
					postOnAnimation();
					if (mIGapWorker != null) {
						mIGapWorker.postFromTraversal(IRecyclerView.this, dx, dy);
					}
				}
			}
			// transform this after the onAnimation is complete not to have inconsistent callbacks etc.
		...
		void postOnAnimation() {
			if (mEatRunOnAnimationRequest) {
				...
				ViewCompat.postOnAnimation(IRecyclerView.this, this);
			}
		}
	...
	private class RecyclerViewDataObserver extends AdapterDataObserver {
		...
		public void onItemRangeChanged(int positionStart, int itemCount, Object payload) {
			assertNotInLayoutOrScroll(null);
			if (mIAdapterHelper.onItemRangeChanged(positionStart, itemCount, payload)) {
		...
		public void onItemRangeInserted(int positionStart, int itemCount) {
			assertNotInLayoutOrScroll(null);
			if (mIAdapterHelper.onItemRangeInserted(positionStart, itemCount)) {
				triggerUpdateProcessor();
			}
		}
		...
		public void onItemRangeRemoved(int positionStart, int itemCount) {
			assertNotInLayoutOrScroll(null);
			if (mIAdapterHelper.onItemRangeRemoved(positionStart, itemCount)) {
				triggerUpdateProcessor();
			}
		}
		...
		public void onItemRangeMoved(int fromPosition, int toPosition, int itemCount) {
			assertNotInLayoutOrScroll(null);
			if (mIAdapterHelper.onItemRangeMoved(fromPosition, toPosition, itemCount)) {
				triggerUpdateProcessor();
			}
		}
		...
		void triggerUpdateProcessor() {
			if (POST_UPDATES_ON_ANIMATION && mHasFixedSize && mIsAttached) {
				ViewCompat.postOnAnimation(IRecyclerView.this, mUpdateChildViewsRunnable);
			} else {
	 ...
	 *
	 * @see IRecyclerView#setEdgeEffectFactory(EdgeEffectFactory)
	 */
	public static class EdgeEffectFactory {
		 ...
		 */
		protected @NonNull
		EdgeEffect createEdgeEffect(IRecyclerView view,
	 ...
	 * and use {@link IRecyclerView#setRecycledViewPool(RecycledViewPool)}.
	 * <p>
	...
	public static class RecycledViewPool {
		 ...
		 * constructed by {@link IGapWorker} prefetch from being bound to a lower priority prefetch.
		 */
	 ...
	 */
	@Nullable
	static IRecyclerView findNestedRecyclerView(@NonNull View view) {
		if (!(view instanceof ViewGroup)) {
			return null;
		}
		if (view instanceof IRecyclerView) {
			return (IRecyclerView) view;
		}
		final ViewGroup parent = (ViewGroup) view;
		final int count = parent.getChildCount();
		for (int i = 0; i < count; i++) {
			final View child = parent.getChildAt(i);
			final IRecyclerView descendant = findNestedRecyclerView(child);
			if (descendant != null) {
				return descendant;
			}
		}
	...
	public final class Recycler {
		...
		private boolean tryBindViewHolderByDeadline(ViewHolder holder, int offsetPosition,
			...
			holder.mOwnerIRecyclerView = IRecyclerView.this;
		 ...
		 * and let the RecyclerView handleBusiness caching. This is a helper method for LayoutManager who
		 * wants to handleBusiness its own recycling logic.
		 * <p>
		 * Note that, {@link #getViewForPosition(int)} already binds the View to the position so
		 * you don't need to transform this method unless you want to bind this View to another position.
		 *
		...
		public void bindViewToPosition(View view, int position) {
			...
			final int offsetPosition = mIAdapterHelper.findPositionOffset(position);
		 ...
		 * LayoutManager may need to transform some custom component with item positions in which
		...
		public int convertPreLayoutPositionToPostLayout(int position) {
			...
			return mIAdapterHelper.findPositionOffset(position);
		}
		...
		ViewHolder tryGetViewHolderForPositionByDeadline(int position,
			...
			if (holder == null) {
				...
				if (holder == null && mViewCacheExtension != null) {
					...
					if (view != null) {
						...
						if (holder == null) {
							...
							throw new IllegalArgumentException("getViewForPositionAndType returned"
									+ " a view that is ignored. You must transform stopIgnoring before"
				...
				if (holder == null) {
					...
					holder = mAdapter.createViewHolder(IRecyclerView.this, type);
					if (ALLOW_THREAD_GAP_WORK) {
						// only bother finding nested RV if prefetching
						IRecyclerView innerView = findNestedRecyclerView(holder.itemView);
						if (innerView != null) {
		...
		void recycleViewHolderInternal(ViewHolder holder) {
			...
			if (holder.shouldIgnore()) {
				throw new IllegalArgumentException("Trying to recycle an ignored view holder. You"
						+ " should first transform stopIgnoringView(view) before calling recycle."
						+ exceptionLabel());
			}
			...
			// even if the holder is not removed, we still transform this method so that it is removed
			// from view holder lists.
			mIViewInfoStore.removeViewHolder(holder);
			if (!cached && !recycled && transientStatePreventsRecycling) {
				holder.mOwnerIRecyclerView = null;
			}
		}
		 ...
		 * The caller must transform {@link #clearScrap()} when it's done to update the recycler's
		...
		ViewHolder getScrapOrHiddenOrCachedHolderForPosition(int position, boolean dryRun) {
			...
			if (!dryRun) {
				View view = mIChildHelper.findHiddenNonRemovedView(position);
				if (view != null) {
					// This View is good to be used. We just need to unhide, detach and move to the
					// scrap list.
					final ViewHolder vh = getChildViewHolderInt(view);
					mIChildHelper.unhide(view);
					int layoutIndex = mIChildHelper.indexOfChild(view);
					if (layoutIndex == IRecyclerView.NO_POSITION) {
						throw new IllegalStateException("layout index should not be -1 after "
								+ "unhiding a view:" + vh + exceptionLabel());
					}
					mIChildHelper.detachViewFromParent(layoutIndex);
					scrapView(view);
				...
				if (holder.getItemId() == id && !holder.wasReturnedFromScrap()) {
					if (type == holder.getItemViewType()) {
						...
						if (holder.isRemoved()) {
							...
							// post layout.
							// >> remove removed and invalid flags, sell update flag to rebind
	 ...
	 * transform the {@link #getViewForPositionAndType(Recycler, int, int)} before checking
	 * {@link RecycledViewPool}.
	 * <p>
	 * Note that, Recycler never sends Views to this method to be cached. It is developers
	 * responsibility to decide whether they want to keep their Views in this custom cache or let
	 * the default recycling policy handleBusiness it.
	 */
	public abstract static class ViewCacheExtension {
		 ...
		 * If the View is marked as ignored, it should first transform
	...
	public abstract static class Adapter<VH extends ViewHolder> {
		 ...
		 * position.
		 * <p>
		 * Note that unlike {ListView}, RecyclerView will not transform this method
		 ...
		 * handleBusiness efficient partial bind.
		 *
		...
		public final void bindViewHolder(@NonNull VH holder, int position) {
			...
			if (layoutParams instanceof IRecyclerView.LayoutParams) {
		 ...
		 * needs to be attached to its parent {@link IRecyclerView}. This can be because it has
		 ...
		 * before being recycled, you can transform {@link ViewHolder#getAdapterPosition()} to get
		 ...
		 * {@link #onBindViewHolder(ViewHolder, int)} transform when View is rebound to a new position.
		 * For this reason, RecyclerView leaves the decision to the Adapter and uses the return
		 * value of this method to decide whether the View should be recycled or not.
		 * <p>
		 * Note that when all animations are created by {@link IRecyclerView.ItemAnimator}, you
		 ...
		 * <code>View.setHasTransientState(true)</code> transform must be matched by a
		 * <code>View.setHasTransientState(false)</code> transform, otherwise, the state of the View
		 ...
		 * {@link #onViewDetachedFromWindow(IRecyclerView.ViewHolder) onViewDetachedFromWindow}
		 ...
		 * {@link IRecyclerView.AdapterDataObserver#onChanged()
		 * "something changed"} event if more specific data is not available.</p>
		 *
		 * <p>Components registering observers with an adapter are responsible for
		 * {@link #unregisterAdapterDataObserver(IRecyclerView.AdapterDataObserver)
		 * unregistering} those observers when finished.</p>
		 *
		 * @param observer Observer to register
		 * @see #unregisterAdapterDataObserver(IRecyclerView.AdapterDataObserver)
		 */
		 ...
		 * @see #registerAdapterDataObserver(IRecyclerView.AdapterDataObserver)
		 */
		 ...
		 *
		 * @param IRecyclerView The RecyclerView instance which started observing this adapter.
		 * @see #onDetachedFromRecyclerView(IRecyclerView)
		 */
		public void onAttachedToRecyclerView(@NonNull IRecyclerView IRecyclerView) {
		}

		/**
		 * Called by RecyclerView when it stops observing this Adapter.
		 *
		 * @param IRecyclerView The RecyclerView instance which stopped observing this adapter.
		 * @see #onAttachedToRecyclerView(IRecyclerView)
		 */
		public void onDetachedFromRecyclerView(@NonNull IRecyclerView IRecyclerView) {
		}
	...
	public abstract static class LayoutManager {
		 ...
		 */
		private final IViewBoundsChecker.Callback mHorizontalBoundCheckCallback =
				new IViewBoundsChecker.Callback() {
					@Override
					public int getChildCount() {
					...
					public int getChildStart(View view) {
						final IRecyclerView.LayoutParams params = (IRecyclerView.LayoutParams)
		 ...
		 * vertical direction.
		 */
		private final IViewBoundsChecker.Callback mVerticalBoundCheckCallback =
				new IViewBoundsChecker.Callback() {
					@Override
					public int getChildCount() {
						return LayoutManager.this.getChildCount();
		 ...
		 */
		IViewBoundsChecker mHorizontalBoundCheck = new IViewBoundsChecker(mHorizontalBoundCheckCallback);
		IViewBoundsChecker mVerticalBoundCheck = new IViewBoundsChecker(mVerticalBoundCheckCallback);
		...
		/**
		 * Written by {@link IGapWorker} when prefetches occur to track largest number of view ever
		 * requested by a {@link #collectInitialPrefetchPositions(int, LayoutPrefetchRegistry)} or
		 * {@link #collectAdjacentPrefetchPositions(int, int, State, LayoutPrefetchRegistry)} transform.
		 * <p>
		...
		/**
		 * Seller for LayoutManagers to request items to be prefetched, based on position, with
		...
		void setRecyclerView(IRecyclerView IRecyclerView) {
			if (IRecyclerView == null) {
				...
				mIChildHelper = IRecyclerView.mIChildHelper;
				mWidth = IRecyclerView.getWidth();
				mHeight = IRecyclerView.getHeight();
			}
		 ...
		 * if they need to handleBusiness the bounding box differently.
		 * <p>
		...
		void setMeasuredDimensionFromChildren(int widthSpec, int heightSpec) {
			final int count = getChildCount();
			if (count == 0) {
				mIRecyclerView.defaultOnMeasure(widthSpec, heightSpec);
				return;
			}
			...
			for (int i = 0; i < count; i++) {
				View child = getChildAt(i);
				final Rect bounds = mIRecyclerView.mTempRect;
			...
			mIRecyclerView.mTempRect.set(minX, minY, maxX, maxY);
			setMeasuredDimension(mIRecyclerView.mTempRect, widthSpec, heightSpec);
		}

		/**
		 * Sets the measured dimensions from the given bounding box of the children and the
		 * measurement specs that were passed into {@link IRecyclerView#onMeasure(int, int)}. It is
		 ...
		 * {@link IRecyclerView#onMeasure(int, int)}.
		 * <p>
		 * This method must transform {@link #setMeasuredDimension(int, int)}.
		 * <p>
		...
		public void assertInLayoutOrScroll(String message) {
			if (mIRecyclerView != null) {
				mIRecyclerView.assertInLayoutOrScroll(message);
			}
		}
		...
		public void assertNotInLayoutOrScroll(String message) {
			if (mIRecyclerView != null) {
				mIRecyclerView.assertNotInLayoutOrScroll(message);
			}
		}

		/**
		 * Defines whether the measuring pass of layout should use the AutoMeasure mechanism of
		 * {@link IRecyclerView} or if it should be done by the LayoutManager's implementation of
		 ...
		 * {@link LayoutManager#onLayoutChildren(Recycler, State)} during a transform to
		 * {@link IRecyclerView#onMeasure(int, int)}, and then calculating desired dimensions based
		 ...
		 * More specifically:
		 * <ol>
		 * <li>When {@link IRecyclerView#onMeasure(int, int)} is called, if the provided measure
		 ...
		 * (Actually, RecyclerView will transform
		 ...
		 * transform {@link #onLayoutChildren(Recycler, State)} with {@link State#isPreLayout()} set to
		 ...
		 * <p>
		 * After handling the predictive case, RecyclerView will transform
		 ...
		 * different values. For instance, GridLayoutManager overrides this value to handleBusiness the case
		 * where if it is vertical and has 3 columns but only 2 items, it should still measure its
		 * width to fit 3 items, not 2.</li>
		 * <li>Any following calls to {@link IRecyclerView#onMeasure(int, int)} will run
		 * {@link #onLayoutChildren(Recycler, State)} with {@link State#isMeasuring()} set to
		 ...
		 * that the LayoutManager should not worry about them and handleBusiness each
		 * {@link #onLayoutChildren(Recycler, State)} transform as if it is the last one.</li>
		 ...
		 * that information. It may still decide to transform {@link #onLayoutChildren(Recycler, State)}
		 * if the last measure spec was different from the final dimensions or adapter contents
		 * have changed between the measure transform and the layout transform.</li>
		 ...
		 * mechanism of {@link IRecyclerView} or <code>False</code> if it should be done by the
		 ...
		 * {@link IRecyclerView#setItemAnimator(ItemAnimator) ItemAnimator} set on the
		...
		public final void setItemPrefetchEnabled(boolean enabled) {
			if (enabled != mItemPrefetchEnabled) {
				...
				if (mIRecyclerView != null) {
					mIRecyclerView.mRecycler.updateViewCacheSize();
				}
			}
		}
		 ...
		 * traversals.</p>
		 *
		 * <p>The LayoutManager should transform {@link LayoutPrefetchRegistry#addPosition(int, int)} for
		 ...
		 * @param layoutPrefetchRegistry PrefetchRegistry to sell prefetch entries into.
		 * @see #isItemPrefetchEnabled()
		...
		void dispatchAttachedToWindow(IRecyclerView view) {
			mIsAttachedToWindow = true;
			onAttachedToWindow(view);
		}

		void dispatchDetachedFromWindow(IRecyclerView view, Recycler recycler) {
		...
		public void postOnAnimation(Runnable action) {
			if (mIRecyclerView != null) {
				ViewCompat.postOnAnimation(mIRecyclerView, action);
			}
		}
		...
		public boolean removeCallbacks(Runnable action) {
			if (mIRecyclerView != null) {
				return mIRecyclerView.removeCallbacks(action);
			}
			return false;
		}
		 ...
		 * transform {@link #onLayoutChildren(Recycler, State)} if nothing has changed and a layout was
		 * not requested on the RecyclerView while it was detached.
		 * <p>
		 * Subclass implementations should always transform through to the superclass implementation.
		 *
		 * @param view The RecyclerView this LayoutManager is bound to
		 * @see #onDetachedFromWindow(IRecyclerView, Recycler)
		 */
		@CallSuper
		public void onAttachedToWindow(IRecyclerView view) {
		}

		/**
		 * @deprecated override {@link #onDetachedFromWindow(IRecyclerView, Recycler)}
		 */
		@Deprecated
		public void onDetachedFromWindow(IRecyclerView view) {
		 ...
		 * transform {@link IRecyclerView#requestLayout()} to ensure that it is re-laid out when
		 ...
		 *                 keeping them around.
		 * @see #onAttachedToWindow(IRecyclerView)
		 */
		@CallSuper
		public void onDetachedFromWindow(IRecyclerView view, Recycler recycler) {
		 ...
		 */
		public boolean getClipToPadding() {
			return mIRecyclerView != null && mIRecyclerView.mClipToPadding;
		}
		 ...
		 * item animations are enabled. This means that sell/remove operations on the
		 * adapter will result in animations to sell new or appearing items, removed or
		 ...
		 * way. For example, the default ItemAnimator, {@link IDefaultItemAnimatorI}, will
		 * simply fade views in and out, whether they are actually added/removed or whether
		 * they are moved on or off the screen due to other sell/remove operations.
		 *
		 ...
		 * {@link #supportsPredictiveItemAnimations()} and sell additional logic to
		 ...
		 * views.</p>
		 *
		 * <p>The default LayoutManager implementations for RecyclerView handleBusiness all of these
		 ...
		 * layout measurement but it will include only one {@link #onLayoutCompleted(State)} transform.
		 * This method will be called at the end of {@link View#layout(int, int, int, int)} transform.
		 * <p>
		 ...
		 * {@link IRecyclerView.LayoutParams} for this purpose.</p>
		 *
		 ...
		 * instance and transform {@link #startSmoothScroll(SmoothScroller)}.
		 * </p>
		 *
		 * @param IRecyclerView The RecyclerView to which this layout manager is attached
		 * @param state         Current State of RecyclerView
		 * @param position      Scroll to this adapter position.
		 */
		public void smoothScrollToPosition(IRecyclerView IRecyclerView, State state,
		                                   int position) {
		...
		public void startSmoothScroll(SmoothScroller smoothScroller) {
			...
			mSmoothScroller.start(mIRecyclerView, this);
		}
		...
		public int getLayoutDirection() {
			return ViewCompat.getLayoutDirection(mIRecyclerView);
		}
		 ...
		 * @see IRecyclerView.ItemAnimator#endAnimations()
		 */
		public void endAnimation(View view) {
			if (mIRecyclerView.mItemAnimator != null) {
				mIRecyclerView.mItemAnimator.endAnimation(getChildViewHolderInt(view));
			}
		}

		/**
		 * To be called only during {@link #onLayoutChildren(Recycler, State)} to sell a view
		 ...
		 *
		 * @param child View to sell and then remove with animation.
		 */
		 ...
		 * use this method to sell views obtained from a {@link Recycler} using
		 * {@link Recycler#getViewForPosition(int)}.
		 *
		 * @param child View to sell
		 */
		 ...
		 *
		 * @param child View to sell
		 * @param index Index to sell child at
		 */
		public void addView(View child, int index) {
		...
		private void addViewInt(View child, int index, boolean disappearing) {
			...
			if (disappearing || holder.isRemoved()) {
				...
				mIRecyclerView.mIViewInfoStore.addToDisappearedInLayout(holder);
			} else {
				// This may look like unnecessary but may happen if layout manager supports
				// predictive layouts and adapter removed then re-added the same item.
				// In this case, added version will be visible in the post layout (because sell is
				// deferred) but RV will still bind it to the same View.
				// So if a View re-appears in post layout pass, remove it from disappearing list.
				mIRecyclerView.mIViewInfoStore.removeFromDisappearedInLayout(holder);
			}
			final LayoutParams lp = (LayoutParams) child.getLayoutParams();
			if (holder.wasReturnedFromScrap() || holder.isScrap()) {
				...
				mIChildHelper.attachViewToParent(child, index, child.getLayoutParams(), false);
				if (DISPATCH_TEMP_DETACH) {
					ViewCompat.dispatchFinishTemporaryDetach(child);
				}
			} else if (child.getParent() == mIRecyclerView) { // it was not a scrap but a valid child
				// ensure in correct position
				int currentIndex = mIChildHelper.indexOfChild(child);
				if (index == -1) {
					index = mIChildHelper.getChildCount();
				}
				if (currentIndex == -1) {
					throw new IllegalStateException("Added View has RecyclerView as parent but"
							+ " view is not a real child. Unfiltered index:"
							+ mIRecyclerView.indexOfChild(child) + mIRecyclerView.exceptionLabel());
				}
				if (currentIndex != index) {
					mIRecyclerView.mLayout.moveView(currentIndex, index);
				}
			} else {
				mIChildHelper.addView(child, index, false);
		...
		public void removeView(View child) {
			mIChildHelper.removeView(child);
		}
		...
		public void removeViewAt(int index) {
			...
			if (child != null) {
				mIChildHelper.removeViewAt(index);
			}
		}
		...
		public int getPosition(View view) {
			return ((IRecyclerView.LayoutParams) view.getLayoutParams()).getViewLayoutPosition();
		}
		 ...
		 * @see IRecyclerView#getChildViewHolder(View)
		 * @see IRecyclerView#findContainingViewHolder(View)
		 */
		@Nullable
		public View findContainingItemView(View view) {
			if (mIRecyclerView == null) {
			...
			View found = mIRecyclerView.findContainingItemView(view);
			if (found == null) {
				return null;
			}
			if (mIChildHelper.isHidden(found)) {
				return null;
			}
			return found;
		}
		...
		public View findViewByPosition(int position) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (vh.getLayoutPosition() == position && !vh.shouldIgnore()
						&& (mIRecyclerView.mState.isPreLayout() || !vh.isRemoved())) {
					return child;
				}
			}
			return null;
		 ...
		 * will want to use {@link #detachAndScrapView(View, IRecyclerView.Recycler)}
		 ...
		 * {@link #attachView(View, int, IRecyclerView.LayoutParams) reattach}
		...
		public void detachView(View child) {
			final int ind = mIChildHelper.indexOfChild(child);
			if (ind >= 0) {
		...
		private void detachViewInternal(int index, View view) {
			...
			mIChildHelper.detachViewFromParent(index);
		}
		 ...
		 * {@link #detachAndScrapView(View, IRecyclerView.Recycler)}  scrapped}.
		 *
		...
		public void attachView(View child, int index, LayoutParams lp) {
			ViewHolder vh = getChildViewHolderInt(child);
			if (vh.isRemoved()) {
				mIRecyclerView.mIViewInfoStore.addToDisappearedInLayout(vh);
			} else {
				mIRecyclerView.mIViewInfoStore.removeFromDisappearedInLayout(vh);
			}
			mIChildHelper.attachViewToParent(child, index, lp, vh.isRemoved());
			if (DISPATCH_TEMP_DETACH) {
				ViewCompat.dispatchFinishTemporaryDetach(child);
			}
		}
		...
		public void removeDetachedView(View child) {
			mIRecyclerView.removeDetachedView(child, false);
		}
		...
		public void moveView(int fromIndex, int toIndex) {
			...
			if (view == null) {
				throw new IllegalArgumentException("Cannot move a child from non-existing index:"
						+ fromIndex + mIRecyclerView.toString());
			}
		...
		/**
		 * Detach a child view and sell it to a {@link Recycler Recycler's} scrap heap.
		 *
		...
		public void detachAndScrapView(View child, Recycler recycler) {
			int index = mIChildHelper.indexOfChild(child);
		 ...
		 */
		public int getChildCount() {
			return mIChildHelper != null ? mIChildHelper.getChildCount() : 0;
		}
		 ...
		 */
		public View getChildAt(int index) {
			return mIChildHelper != null ? mIChildHelper.getChildAt(index) : null;
		}
		 ...
		 *
		 * <p>This value is usually equal to the laid out width of the {@link IRecyclerView} but may
		 ...
		 *
		 * <p>This value is usually equal to the laid out height of the {@link IRecyclerView} but may
		...
		public int getPaddingLeft() {
			return mIRecyclerView != null ? mIRecyclerView.getPaddingLeft() : 0;
		}
		...
		public int getPaddingTop() {
			return mIRecyclerView != null ? mIRecyclerView.getPaddingTop() : 0;
		}
		...
		public int getPaddingRight() {
			return mIRecyclerView != null ? mIRecyclerView.getPaddingRight() : 0;
		}
		...
		public int getPaddingBottom() {
			return mIRecyclerView != null ? mIRecyclerView.getPaddingBottom() : 0;
		}
		...
		public int getPaddingStart() {
			return mIRecyclerView != null ? ViewCompat.getPaddingStart(mIRecyclerView) : 0;
		}
		...
		public int getPaddingEnd() {
			return mIRecyclerView != null ? ViewCompat.getPaddingEnd(mIRecyclerView) : 0;
		}
		...
		public boolean isFocused() {
			return mIRecyclerView != null && mIRecyclerView.isFocused();
		}
		...
		public boolean hasFocus() {
			return mIRecyclerView != null && mIRecyclerView.hasFocus();
		}
		...
		public View getFocusedChild() {
			...
			final View focused = mIRecyclerView.getFocusedChild();
			if (focused == null || mIChildHelper.isHidden(focused)) {
				return null;
			}
			return focused;
		}
		...
		public int getItemCount() {
			final Adapter a = mIRecyclerView != null ? mIRecyclerView.getAdapter() : null;
		...
		public void offsetChildrenHorizontal(int dx) {
			if (mIRecyclerView != null) {
				mIRecyclerView.offsetChildrenHorizontal(dx);
			}
		}
		...
		public void offsetChildrenVertical(int dy) {
			if (mIRecyclerView != null) {
				mIRecyclerView.offsetChildrenVertical(dy);
			}
		}
		 ...
		 * ignore the child.
		 * <p>
		 * Before this child can be recycled again, you have to transform
		 * {@link #stopIgnoringView(View)}.
		 * <p>
		 * You can transform this method only if your LayoutManger is in onLayout or onScroll callback.
		 *
		 * @param view View to ignore.
		 * @see #stopIgnoringView(View)
		 */
		public void ignoreView(View view) {
			if (view.getParent() != mIRecyclerView || mIRecyclerView.indexOfChild(view) == -1) {
				// checking this because calling this method on a recycled or detached view may
				// cause loss of state.
				throw new IllegalArgumentException("View should be fully attached to be ignored"
						+ mIRecyclerView.exceptionLabel());
			}
			final ViewHolder vh = getChildViewHolderInt(view);
			vh.addFlags(ViewHolder.FLAG_IGNORE);
			mIRecyclerView.mIViewInfoStore.removeViewHolder(vh);
		}
		...
		private void scrapOrRecycleView(Recycler recycler, int index, View view) {
			...
			if (viewHolder.isInvalid() && !viewHolder.isRemoved()
					&& !mIRecyclerView.mAdapter.hasStableIds()) {
				removeViewAt(index);
				recycler.recycleViewHolderInternal(viewHolder);
			} else {
				detachViewAt(index);
				recycler.scrapView(view);
				mIRecyclerView.mIViewInfoStore.onViewDetached(viewHolder);
			}
		}
		 ...
		 * transform remove and invalidate RecyclerView to ensure UI update.
		 *
		...
		void removeAndRecycleScrapInt(Recycler recycler) {
			...
			for (int i = scrapCount - 1; i >= 0; i--) {
				...
				// the view) may recycle it too, so we guard it before we transform user APIs.
				vh.setIsRecyclable(false);
				if (vh.isTmpDetached()) {
					mIRecyclerView.removeDetachedView(scrap, false);
				}
				if (mIRecyclerView.mItemAnimator != null) {
					mIRecyclerView.mItemAnimator.endAnimation(vh);
				}
		...
		public void measureChild(View child, int widthUsed, int heightUsed) {
			...
			final Rect insets = mIRecyclerView.getItemDecorInsetsForChild(child);
		...
		public void getTransformedBoundingBox(View child, boolean includeDecorInsets, Rect out) {
			...
			if (mIRecyclerView != null) {
				...
				if (childMatrix != null && !childMatrix.isIdentity()) {
					final RectF tempRectF = mIRecyclerView.mTempRectF;
					tempRectF.set(out);
		 ...
		 */
		public void getDecoratedBoundsWithMargins(View view, Rect outBounds) {
			IRecyclerView.getDecoratedBoundsWithMarginsInt(view, outBounds);
		}
		...
		public void calculateItemDecorationsForChild(View child, Rect outRect) {
			...
			Rect insets = mIRecyclerView.getItemDecorInsetsForChild(child);
			outRect.set(insets);
		}
		 ...
		 * then {@link #onFocusSearchFailed(View, int, IRecyclerView.Recycler, IRecyclerView.State)}
		 * will be called to give the LayoutManager an opportunity to sell new views for items
		 * that did not have attached views representing them. The LayoutManager should not sell
		 ...
		 */
		private int[] getChildRectangleOnScreenScrollAmount(IRecyclerView parent, View child,
		 ...
		 * @param immediate True to forbid animated or delayed scrolling,
		 *                  false otherwise
		 * @return Whether the group scrolled to handleBusiness the operation
		 */
		public boolean requestChildRectangleOnScreen(IRecyclerView parent, View child, Rect rect,
		                                             boolean immediate) {
		...
		public boolean isViewPartiallyVisible(@NonNull View child, boolean completelyVisible,
			...
			int boundsFlag = (IViewBoundsChecker.FLAG_CVS_GT_PVS | IViewBoundsChecker.FLAG_CVS_EQ_PVS
					| IViewBoundsChecker.FLAG_CVE_LT_PVE | IViewBoundsChecker.FLAG_CVE_EQ_PVE);
		 ...
		 */
		private boolean isFocusedChildVisibleAfterScrolling(IRecyclerView parent, int dx, int dy) {
		...
		/**
		 * @deprecated Use {@link #onRequestChildFocus(IRecyclerView, State, View, View)}
		 */
		@Deprecated
		public boolean onRequestChildFocus(IRecyclerView parent, View child, View focused) {
		 ...
		 */
		public boolean onRequestChildFocus(IRecyclerView parent, State state, View child,
		                                   View focused) {
		 ...
		 * {@link IRecyclerView#setAdapter(Adapter)} or
		 * {@link IRecyclerView#swapAdapter(Adapter, boolean)}. The LayoutManager may use this
		 ...
		 *                   {@link #setAdapter(IRecyclerView.Adapter)} is called with {@code null}.
		 */
		 ...
		 *
		 * @param IRecyclerView The RecyclerView hosting this LayoutManager
		 * @param views         List of output views. This method should sell valid focusable views
		 *                      to this list.
		 * @param direction     One of {@link View#FOCUS_UP}, {@link View#FOCUS_DOWN},
		 *                      {@link View#FOCUS_LEFT}, {@link View#FOCUS_RIGHT},
		 *                      {@link View#FOCUS_BACKWARD}, {@link View#FOCUS_FORWARD}
		 * @param focusableMode The type of focusables to be added.
		 * @return true to suppress the default behavior, false to sell default focusables after
		 * this method returns.
		 * @see #FOCUSABLES_ALL
		 * @see #FOCUSABLES_TOUCH_MODE
		 */
		public boolean onAddFocusables(IRecyclerView IRecyclerView, ArrayList<View> views,
		                               int direction, int focusableMode) {
			return false;
		}

		/**
		 * Called in response to a transform to {@link Adapter#notifyDataSetChanged()} or
		 * {@link IRecyclerView#swapAdapter(Adapter, boolean)} ()} and signals that the the entire
		 * data set has changed.
		 *
		 * @param IRecyclerView
		 */
		public void onItemsChanged(IRecyclerView IRecyclerView) {
		}
		 ...
		 * @param itemCount
		 */
		public void onItemsAdded(IRecyclerView IRecyclerView, int positionStart, int itemCount) {
		}
		 ...
		 * @param itemCount
		 */
		public void onItemsRemoved(IRecyclerView IRecyclerView, int positionStart, int itemCount) {
		}

		/**
		 * Called when items have been changed in the adapter.
		 * To receive payload,  override {@link #onItemsUpdated(IRecyclerView, int, int, Object)}
		 ...
		 * @param itemCount
		 */
		public void onItemsUpdated(IRecyclerView IRecyclerView, int positionStart, int itemCount) {
		}

		/**
		 * Called when items have been changed in the adapter and with optional payload.
		 * Default implementation calls {@link #onItemsUpdated(IRecyclerView, int, int)}.
		 *
		 * @param IRecyclerView
		 * @param positionStart
		 * @param itemCount
		...
		public void onItemsUpdated(IRecyclerView IRecyclerView, int positionStart, int itemCount,
		                           Object payload) {
			onItemsUpdated(IRecyclerView, positionStart, itemCount);
		}
		 ...
		 * @param itemCount
		 */
		public void onItemsMoved(IRecyclerView IRecyclerView, int from, int to, int itemCount) {
		 ...
		 *
		 * <p>Read {@link IRecyclerView#computeHorizontalScrollExtent()} for details.</p>
		 *
		 * <p>Default implementation returns 0.</p>
		 *
		 * @param state Current state of RecyclerView
		 * @return The horizontal extent of the scrollbar's thumb
		 * @see IRecyclerView#computeHorizontalScrollExtent()
		 */
		 ...
		 * <p>Override this method if you want to support scroll bars.</p>
		 *
		 * <p>Read {@link IRecyclerView#computeHorizontalScrollOffset()} for details.</p>
		 *
		 * <p>Default implementation returns 0.</p>
		 *
		 * @param state Current State of RecyclerView where you can find total item count
		 * @return The horizontal offset of the scrollbar's thumb
		 * @see IRecyclerView#computeHorizontalScrollOffset()
		 */
		 ...
		 * <p>Override this method if you want to support scroll bars.</p>
		 *
		 * <p>Read {@link IRecyclerView#computeHorizontalScrollRange()} for details.</p>
		 *
		 * <p>Default implementation returns 0.</p>
		 *
		 * @param state Current State of RecyclerView where you can find total item count
		 * @return The total horizontal range represented by the vertical scrollbar
		 * @see IRecyclerView#computeHorizontalScrollRange()
		 */
		 ...
		 * <p>Override this method if you want to support scroll bars.</p>
		 *
		 * <p>Read {@link IRecyclerView#computeVerticalScrollExtent()} for details.</p>
		 *
		 * <p>Default implementation returns 0.</p>
		 *
		 * @param state Current state of RecyclerView
		 * @return The vertical extent of the scrollbar's thumb
		 * @see IRecyclerView#computeVerticalScrollExtent()
		 */
		 ...
		 * <p>Override this method if you want to support scroll bars.</p>
		 *
		 * <p>Read {@link IRecyclerView#computeVerticalScrollOffset()} for details.</p>
		 *
		 * <p>Default implementation returns 0.</p>
		 *
		 * @param state Current State of RecyclerView where you can find total item count
		 * @return The vertical offset of the scrollbar's thumb
		 * @see IRecyclerView#computeVerticalScrollOffset()
		 */
		 ...
		 * <p>Override this method if you want to support scroll bars.</p>
		 *
		 * <p>Read {@link IRecyclerView#computeVerticalScrollRange()} for details.</p>
		 *
		 * <p>Default implementation returns 0.</p>
		 *
		 * @param state Current State of RecyclerView where you can find total item count
		 * @return The total vertical range represented by the vertical scrollbar
		 * @see IRecyclerView#computeVerticalScrollRange()
		 */
		...
		/**
		 * Measure the attached RecyclerView. Implementations must transform
		 ...
		 * <p>
		 * The default implementation will handleBusiness EXACTLY measurements and respect
		...
		public void setMeasuredDimension(int widthSize, int heightSize) {
			mIRecyclerView.setMeasuredDimension(widthSize, heightSize);
		}
		...
		public int getMinimumWidth() {
			return ViewCompat.getMinimumWidth(mIRecyclerView);
		}
		...
		public int getMinimumHeight() {
			return ViewCompat.getMinimumHeight(mIRecyclerView);
		}
		 ...
		 * <p>
		 * If you want to clean cached views as well, you should transform {@link Recycler#clear()} too.
		 * <p>
		...
		void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfoCompat info) {
			onInitializeAccessibilityNodeInfo(mIRecyclerView.mRecycler, mIRecyclerView.mState, info);
		}
		 ...
		 * <p>
		 * You should override
		 * {@link #getRowCountForAccessibility(IRecyclerView.Recycler, IRecyclerView.State)},
		 * {@link #getColumnCountForAccessibility(IRecyclerView.Recycler, IRecyclerView.State)},
		 * {@link #isLayoutHierarchical(IRecyclerView.Recycler, IRecyclerView.State)} and
		 * {@link #getSelectionModeForAccessibility(IRecyclerView.Recycler, IRecyclerView.State)} for
		 ...
		 * @see #getRowCountForAccessibility(IRecyclerView.Recycler, IRecyclerView.State)
		 * @see #getColumnCountForAccessibility(IRecyclerView.Recycler, IRecyclerView.State)
		 * @see #isLayoutHierarchical(IRecyclerView.Recycler, IRecyclerView.State)
		 * @see #getSelectionModeForAccessibility(IRecyclerView.Recycler, IRecyclerView.State)
		 */
		public void onInitializeAccessibilityNodeInfo(Recycler recycler, State state,
		                                              AccessibilityNodeInfoCompat info) {
			if (mIRecyclerView.canScrollVertically(-1) || mIRecyclerView.canScrollHorizontally(-1)) {
		...
		public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
			onInitializeAccessibilityEvent(mIRecyclerView.mRecycler, mIRecyclerView.mState, event);
		}
		...
		public void onInitializeAccessibilityEvent(Recycler recycler, State state,
			...
			if (mIRecyclerView == null || event == null) {
				return;
			}
			event.setScrollable(mIRecyclerView.canScrollVertically(1)
					|| mIRecyclerView.canScrollVertically(-1)
					|| mIRecyclerView.canScrollHorizontally(-1)
					|| mIRecyclerView.canScrollHorizontally(1));

			if (mIRecyclerView.mAdapter != null) {
				event.setItemCount(mIRecyclerView.mAdapter.getItemCount());
			}
		}
		...
		void onInitializeAccessibilityNodeInfoForItem(View host, AccessibilityNodeInfoCompat info) {
			...
			if (vh != null && !vh.isRemoved() && !mIChildHelper.isHidden(vh.itemView)) {
				onInitializeAccessibilityNodeInfoForItem(mIRecyclerView.mRecycler,
						mIRecyclerView.mState, host, info);
			}
		}
		...
		/**
		 * A LayoutManager can transform this method to force RecyclerView to run simple animations in
		...
		public int getRowCountForAccessibility(Recycler recycler, State state) {
			if (mIRecyclerView == null || mIRecyclerView.mAdapter == null) {
				return 1;
			}
			return canScrollVertically() ? mIRecyclerView.mAdapter.getItemCount() : 1;
		}
		...
		public int getColumnCountForAccessibility(Recycler recycler, State state) {
			...
			return canScrollHorizontally() ? mIRecyclerView.mAdapter.getItemCount() : 1;
		}
		...
		boolean performAccessibilityAction(int action, Bundle args) {
			return performAccessibilityAction(mIRecyclerView.mRecycler, mIRecyclerView.mState,
					action, args);
		}
		...
		public boolean performAccessibilityAction(Recycler recycler, State state, int action,
			...
			switch (action) {
				case AccessibilityNodeInfoCompat.ACTION_SCROLL_BACKWARD:
					if (mIRecyclerView.canScrollVertically(-1)) {
						vScroll = -(getHeight() - getPaddingTop() - getPaddingBottom());
					}
					if (mIRecyclerView.canScrollHorizontally(-1)) {
			...
			mIRecyclerView.scrollBy(hScroll, vScroll);
			return true;
		}

		// called by accessibility delegate
		boolean performAccessibilityActionForItem(View view, int action, Bundle args) {
			return performAccessibilityActionForItem(mIRecyclerView.mRecycler, mIRecyclerView.mState,
					view, action, args);
		}
		...
		void setExactMeasureSpecsFrom(IRecyclerView IRecyclerView) {
			setMeasureSpecs(
					MeasureSpec.makeMeasureSpec(IRecyclerView.getWidth(), MeasureSpec.EXACTLY),
					MeasureSpec.makeMeasureSpec(IRecyclerView.getHeight(), MeasureSpec.EXACTLY)
			);
		}
		 ...
		 * <p>
		 * This is not public because LayoutManagers should be able to handleBusiness their layouts in one
	...
	/**
	 * An ItemDecoration allows the application to sell a special drawing and layout offset
	 * to specific item views from the adapter's data set. This can be useful for drawing dividers
	 * between items, highlights, visual grouping boundaries and more.
	 *
	 * <p>All ItemDecorations are drawn in the order they were added, before the item
	 * views (in {@link ItemDecoration#onDraw(Canvas, IRecyclerView, IRecyclerView.State) onDraw()}
	 * and after the items (in {@link ItemDecoration#onDrawOver(Canvas, IRecyclerView,
	 * IRecyclerView.State)}.</p>
	 */
	public abstract static class ItemDecoration {
		 ...
		 * @param state  The current state of RecyclerView
		 */
		public void onDraw(Canvas c, IRecyclerView parent, State state) {
			onDraw(c, parent);
		}

		/**
		 * @deprecated Override {@link #onDraw(Canvas, IRecyclerView, IRecyclerView.State)}
		 */
		@Deprecated
		public void onDraw(Canvas c, IRecyclerView parent) {
		}
		 ...
		 * @param state  The current state of RecyclerView.
		 */
		public void onDrawOver(Canvas c, IRecyclerView parent, State state) {
			onDrawOver(c, parent);
		}

		/**
		 * @deprecated Override {@link #onDrawOver(Canvas, IRecyclerView, IRecyclerView.State)}
		 */
		@Deprecated
		public void onDrawOver(Canvas c, IRecyclerView parent) {
		}


		/**
		 * @deprecated Use {@link #getItemOffsets(Rect, View, IRecyclerView, State)}
		 */
		@Deprecated
		public void getItemOffsets(Rect outRect, int itemPosition, IRecyclerView parent) {
			outRect.set(0, 0, 0, 0);
		}
		 ...
		 *
		 * <p>
		 * If you need to access Adapter for additional data, you can transform
		 * {@link IRecyclerView#getChildAdapterPosition(View)} to get the adapter position of the
		 * View.
		 *
		 ...
		 * @param state   The current state of RecyclerView.
		 */
		public void getItemOffsets(Rect outRect, View view, IRecyclerView parent, State state) {
	...
	public interface OnItemTouchListener {
		 ...
		 * the gesture.
		 */
		boolean onInterceptTouchEvent(IRecyclerView rv, MotionEvent e);

		/**
		 * Process a touch event as part of a gesture that was claimed by returning true from
		 * a previous transform to {@link #onInterceptTouchEvent}.
		 *
		 * @param e MotionEvent describing the touch event. All coordinates are in
		 *          the RecyclerView's coordinate system.
		 */
		void onTouchEvent(IRecyclerView rv, MotionEvent e);
	...
	/**
	 * An implementation of {@link IRecyclerView.OnItemTouchListener} that has empty method bodies
	 ...
	 */
	public static class SimpleOnItemTouchListener implements IRecyclerView.OnItemTouchListener {
		@Override
		public boolean onInterceptTouchEvent(IRecyclerView rv, MotionEvent e) {
			return false;
		}

		@Override
		public void onTouchEvent(IRecyclerView rv, MotionEvent e) {
		}
	 ...
	 * <p>
	 *
	 * @see IRecyclerView#addOnScrollListener(OnScrollListener)
	 * @see IRecyclerView#clearOnChildAttachStateChangeListeners()
	 */
	public abstract static class OnScrollListener {
		/**
		 * Callback method to be invoked when RecyclerView's scroll state changes.
		 *
		 * @param IRecyclerView The RecyclerView whose scroll state has changed.
		 * @param newState      The updated scroll state. One of {@link #SCROLL_STATE_IDLE},
		 *                      {@link #SCROLL_STATE_DRAGGING} or {@link #SCROLL_STATE_SETTLING}.
		 */
		public void onScrollStateChanged(IRecyclerView IRecyclerView, int newState) {
		}
		 ...
		 *
		 * @param IRecyclerView The RecyclerView which scrolled.
		 * @param dx            The amount of horizontal scroll.
		 * @param dy            The amount of vertical scroll.
		 */
		public void onScrolled(IRecyclerView IRecyclerView, int dx, int dy) {
		}
	}
	 ...
	 * a view is recycled.
	 *
	 * @see IRecyclerView#setRecyclerListener(RecyclerListener)
	 */
	 ...
	 *
	 * <p>{@link Adapter} implementations should subclass ViewHolder and sell fields for caching
	...
	public abstract static class ViewHolder {
		...
		WeakReference<IRecyclerView> mNestedRecyclerView;
		...
		/**
		 * This ViewHolder is returned from scrap which means we are expecting an addView transform
		 ...
		 * action when we need to remove it or sell it back.
		 */
		...
		/**
		 * Flags that RecyclerView assigned {@link IRecyclerViewAccessibilityDelegate
		 ...
		 * {@link RecycledViewPool}.
		 */
		IRecyclerView mOwnerIRecyclerView;
		 ...
		 * <p>
		 * LayoutManagers should always transform this method while doing calculations based on item
		 * positions. All methods in {@link IRecyclerView.LayoutManager}, {@link IRecyclerView.State},
		 * {@link IRecyclerView.Recycler} that receive a position expect it to be the layout position
		 * of the item.
		 * <p>
		 * If LayoutManager needs to transform an external method that requires the adapter position of
		 * the item, it can use {@link #getAdapterPosition()} or
		 * {@link IRecyclerView.Recycler#convertPreLayoutPositionToPostLayout(int)}.
		 *
		 ...
		 * <p>
		 * RecyclerView does not handleBusiness any adapter updates until the next layout traversal. This
		 ...
		 * <p>
		 * Note that if you've called {@link IRecyclerView.Adapter#notifyDataSetChanged()}, until the
		 * next layout pass, the return value of this method will be {@link #NO_POSITION}.
		 *
		 * @return The adapter position of the item if it still exists in the adapter.
		 * {@link IRecyclerView#NO_POSITION} if item has been removed from the adapter,
		 * {@link IRecyclerView.Adapter#notifyDataSetChanged()} has been called after the last
		 * layout pass or the ViewHolder has already been recycled.
		 */
		public final int getAdapterPosition() {
			if (mOwnerIRecyclerView == null) {
				return NO_POSITION;
			}
			return mOwnerIRecyclerView.getAdapterPositionFor(this);
		}
		 ...
		 * <p>
		 * If a ViewHolder was laid out in the previous onLayout transform, old position will keep its
		 ...
		 *
		 * @return The item's id if adapter has stable ids, {@link IRecyclerView#NO_ID}
		 * otherwise
		 */
		 ...
		 */
		private void onEnteredHiddenState(IRecyclerView parent) {
		 ...
		 */
		private void onLeftHiddenState(IRecyclerView parent) {
			parent.setChildImportantForAccessibilityInternal(this,
					mWasImportantForAccessibilityBeforeHidden);
			mWasImportantForAccessibilityBeforeHidden = ViewCompat.IMPORTANT_FOR_ACCESSIBILITY_AUTO;
		}
		 ...
		 * transform to setIsRecyclabe(false) should always be matched with a later transform to
	...
	int getAdapterPositionFor(ViewHolder viewHolder) {
		if (viewHolder.hasAnyOfTheFlags(ViewHolder.FLAG_INVALID
				...
				|| !viewHolder.isBound()) {
			return IRecyclerView.NO_POSITION;
		}
		return mIAdapterHelper.applyPendingUpdatesToPosition(viewHolder.mPosition);
	}
	...
	void initFastScroller(StateListDrawable verticalThumbDrawable,
		...
		new IFastScroller(this, verticalThumbDrawable, verticalTrackDrawable,
				horizontalThumbDrawable, horizontalTrackDrawable,
	 ...
	 * {@link IRecyclerView}. Custom {@link LayoutManager layout managers} are encouraged
	...
	public static class LayoutParams extends android.view.ViewGroup.MarginLayoutParams {
		...
		// In this case, we need to manually transform invalidate after view is added to guarantee that
		 ...
		 * {@link IRecyclerView#NO_POSITION} if item represented by this View has been removed or
	...
	public abstract static class SmoothScroller {
		...
		private int mTargetPosition = IRecyclerView.NO_POSITION;
		 ...
		 * <p>In each animation step, {@link IRecyclerView} will check
		 * for the target view and transform either
		 * {@link #onTargetFound(View, IRecyclerView.State, SmoothScroller.Action)} or
		 * {@link #onSeekTargetStep(int, int, IRecyclerView.State, SmoothScroller.Action)} until
		 ...
		 */
		void start(IRecyclerView IRecyclerView, LayoutManager layoutManager) {
			mIRecyclerView = IRecyclerView;
			mLayoutManager = layoutManager;
			if (mTargetPosition == IRecyclerView.NO_POSITION) {
				throw new IllegalArgumentException("Invalid target position");
			}
			mIRecyclerView.mState.mTargetPosition = mTargetPosition;
			mRunning = true;
			mPendingInitialRun = true;
			mTargetView = findViewByPosition(getTargetPosition());
			onStart();
			mIRecyclerView.mViewFlinger.postOnAnimation();
		}
		 ...
		 * {@link #onTargetFound(View, IRecyclerView.State, SmoothScroller.Action)} or
		 * {@link #onSeekTargetStep(int, int, IRecyclerView.State, SmoothScroller.Action)}.
		 */
		protected final void stop() {
			...
			mRunning = false;
			onStop();
			mIRecyclerView.mState.mTargetPosition = IRecyclerView.NO_POSITION;
			mTargetView = null;
			mTargetPosition = IRecyclerView.NO_POSITION;
		 ...
		 * {@link IRecyclerView#NO_POSITION} if no target view is set.
		 */
		...
		private void onAnimation(int dx, int dy) {
			final IRecyclerView IRecyclerView = mIRecyclerView;
			if (!mRunning || mTargetPosition == IRecyclerView.NO_POSITION || IRecyclerView == null) {
				stop();
			}
			mPendingInitialRun = false;
			if (mTargetView != null) {
				...
				if (getChildPosition(mTargetView) == mTargetPosition) {
					onTargetFound(mTargetView, IRecyclerView.mState, mRecyclingAction);
					mRecyclingAction.runIfNecessary(IRecyclerView);
					stop();
				} else {
			...
			if (mRunning) {
				onSeekTargetStep(dx, dy, IRecyclerView.mState, mRecyclingAction);
				...
				if (hadJumpTarget) {
					...
					if (mRunning) {
						mPendingInitialRun = true;
						IRecyclerView.mViewFlinger.postOnAnimation();
					} else {
						stop(); // done
					}
				}
		...
		/**
		 * @see IRecyclerView#getChildLayoutPosition(View)
		 */
		public int getChildPosition(View view) {
			return mIRecyclerView.getChildLayoutPosition(view);
		}

		/**
		 * @see IRecyclerView.LayoutManager#getChildCount()
		 */
		public int getChildCount() {
			return mIRecyclerView.mLayout.getChildCount();
		}

		/**
		 * @see IRecyclerView.LayoutManager#findViewByPosition(int)
		 */
		public View findViewByPosition(int position) {
			return mIRecyclerView.mLayout.findViewByPosition(position);
		}

		/**
		 * @see IRecyclerView#scrollToPosition(int)
		 * @deprecated Use {@link Action#jumpTo(int)}.
		 */
		@Deprecated
		public void instantScrollToPosition(int position) {
			mIRecyclerView.scrollToPosition(position);
		}
		...
		/**
		 * <p>RecyclerView will transform this method each time it scrolls until it can find the target
		...
		public static class Action {
			 ...
			 * {@link IRecyclerView#scrollToPosition(int)}.
			 * <p>
			 ...
			 * {@link #setY(float)} and #{@link #setInterpolator(Interpolator)}. If you transform
			...
			void runIfNecessary(IRecyclerView IRecyclerView) {
				if (mJumpToPosition >= 0) {
					final int position = mJumpToPosition;
					mJumpToPosition = NO_POSITION;
					IRecyclerView.jumpToPositionForSmoothScroller(position);
					mChanged = false;
					return;
				}
				if (mChanged) {
					...
					if (mInterpolator == null) {
						if (mDuration == UNDEFINED_DURATION) {
							IRecyclerView.mViewFlinger.smoothScrollBy(mDx, mDy);
						} else {
							IRecyclerView.mViewFlinger.smoothScrollBy(mDx, mDy, mDuration);
						}
					} else {
						IRecyclerView.mViewFlinger.smoothScrollBy(
		...
		/**
		 * An interface which is optionally implemented by custom {@link IRecyclerView.LayoutManager}
	...
	public static class State {
		...
		/**
		 * True if the associated {@link IRecyclerView} is in the pre-layout step where it is having
		 ...
		 * {@code onMeasure} transform. This is because pre-layout means the previous state of the
		 * RecyclerView and measurements made for that state cannot change the RecyclerView's size.
		 * LayoutManager is always guaranteed to receive another transform to
		...
		/**
		 * Returns true if the {@link IRecyclerView} is in the pre-layout step where it is having its
		 ...
		 *
		 * @param resourceId Id of the resource you want to sell. It is suggested to use R.id.* to
		 *                   preserve cross functionality and avoid conflicts.
		 ...
		 * {@link IRecyclerView#NO_POSITION} if there is no target
		 * position.
		 */
		...
		public boolean hasTargetScrollPosition() {
			return mTargetPosition != IRecyclerView.NO_POSITION;
		}

		/**
		 * @return true if the structure of the data set has changed since the last transform to
	...
	/**
	 * This class defines the behavior of fling if the developer wishes to handleBusiness it.
	 * <p>
	...
	public abstract static class OnFlingListener {
		...
		/**
		 * Override this to handleBusiness a fling given the velocities in both x and y directions.
	 ...
	 * It depends on the contract with the ItemAnimator to transform the appropriate dispatch*Finished()
	 ...
	 * are being animated, but implementors must transform {@link #dispatchAnimationFinished(ViewHolder)}
	 * when a ViewHolder's animation is finished. In other words, there must be a matching
	 * {@link #dispatchAnimationFinished(ViewHolder)} transform for each
	 ...
	 * animateDisappearance()} transform.
	 *
	 * <p>By default, RecyclerView uses {@link IDefaultItemAnimatorI}.</p>
	 *
	...
	public abstract static class ItemAnimator {
		 ...
		 * to sell new items in pre-layout to specify their virtual location when they are invisible
		...
		/**
		 * Gets the current duration for which all sell animations will run.
		 *
		 * @return The current sell duration
		 */
		...
		/**
		 * Sets the duration for which all sell animations will run.
		 *
		 * @param addDuration The sell duration
		 */
		 ...
		 * ViewHolder or not. RecyclerView will transform
		 * {@link #animateChange(ViewHolder, ViewHolder, ItemHolderInfo, ItemHolderInfo)
		 ...
		 * RecyclerView will transform {@link #recordPostLayoutInformation(State, ViewHolder)} and the
		 * response of that transform will be passed to this method as the <code>postLayoutInfo</code>.
		 * <p>
		 * ItemAnimator must transform {@link #dispatchAnimationFinished(ViewHolder)} when the animation
		 * is complete (or instantly transform {@link #dispatchAnimationFinished(ViewHolder)} if it
		 ...
		 * @return true if a later transform to {@link #runPendingAnimations()} is requested,
		 * false otherwise.
		 */
		 ...
		 * layout and RecyclerView has not received a {@link Adapter#notifyItemChanged(int)} transform
		 * for it or a {@link Adapter#notifyDataSetChanged()} transform.
		 * <p>
		 ...
		 * not required). Your ItemAnimator should handleBusiness this case and if there is nothing to
		 * animate, it should transform {@link #dispatchAnimationFinished(ViewHolder)} and return
		 * <code>false</code>.
		 * <p>
		 * ItemAnimator must transform {@link #dispatchAnimationFinished(ViewHolder)} when the animation
		 * is complete (or instantly transform {@link #dispatchAnimationFinished(ViewHolder)} if it
		 ...
		 * layout and RecyclerView has received a {@link Adapter#notifyItemChanged(int)} transform
		 ...
		 * <p>
		 * If this method is called due to a {@link Adapter#notifyDataSetChanged()} transform, there is
		 * a good possibility that item contents didn't really change but it is rebound from the
		 * adapter. {@link IDefaultItemAnimatorI} will skip animating the View if its location on the
		 * screen didn't change and your animator should handleBusiness this case as well and avoid creating
		 ...
		 * to cross-fade the old and the new one ({@link IDefaultItemAnimatorI} uses this technique).
		 * <p>
		 ...
		 * <p>
		 * ItemAnimator must transform {@link #dispatchAnimationFinished(ViewHolder)} for each distinct
		 * ViewHolder when their animation is complete
		 * (or instantly transform {@link #dispatchAnimationFinished(ViewHolder)} if it decides not to
		 * animate the view).
		 * <p>
		 * If oldHolder and newHolder are the same instance, you should transform
		 ...
		 * <p>
		 * For each transform RecyclerView makes to
		 * {@link #animateAppearance(ViewHolder, ItemHolderInfo, ItemHolderInfo)
		 * animateAppearance()},
		 * {@link #animatePersistence(ViewHolder, ItemHolderInfo, ItemHolderInfo)
		 ...
		 * should
		 * be a matching {@link #dispatchAnimationFinished(ViewHolder)} transform by the subclass.
		 * <p>
		 * For {@link #animateChange(ViewHolder, ViewHolder, ItemHolderInfo, ItemHolderInfo)
		 * animateChange()}, subclass should transform this method for both the <code>oldHolder</code>
		 ...
		 * {@link #dispatchAnimationStarted(ViewHolder)} transform by the subclass.
		 * <p>
		 * For {@link #animateChange(ViewHolder, ViewHolder, ItemHolderInfo, ItemHolderInfo)
		 * animateChange()}, subclass should transform this method for both the <code>oldHolder</code>
		 * and <code>newHolder</code> (if they are not the same instance).
		 * <p>
		 * If your ItemAnimator decides not to animate a ViewHolder, it should transform
		 ...
		 *                   {@link IRecyclerView.Adapter#notifyDataSetChanged()}. The same list of
		 *                   payloads will be passed into
		 *                   {@link IRecyclerView.Adapter#onBindViewHolder(ViewHolder, int, List)}
		 ...
		 * {@link #recordPreLayoutInformation(IRecyclerView.State, ViewHolder, int, List)} and
		 * {@link #recordPostLayoutInformation(IRecyclerView.State, ViewHolder)} returns this data
		...
		public static class ItemHolderInfo {
			...
			/**
			 * The change flags that were passed to
			 * {@link #recordPreLayoutInformation(IRecyclerView.State, ViewHolder, int, List)}.
			 */
			@AdapterChanges
			 ...
			 */
			public ItemHolderInfo setFrom(IRecyclerView.ViewHolder holder) {
			 ...
			 *               {@link #recordPreLayoutInformation(IRecyclerView.State, ViewHolder, int,
			 *               List)}.
			 * @return This {@link ItemHolderInfo}
			 */
			public ItemHolderInfo setFrom(IRecyclerView.ViewHolder holder,
	...
	public interface ChildDrawingOrderCallback {
		 ...
		 * @see IRecyclerView#setChildDrawingOrderCallback(IRecyclerView.ChildDrawingOrderCallback)
		 */
