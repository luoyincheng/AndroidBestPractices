package yincheng.tinytank.sourcecode.viewdraghelper;
...
/**
 * Created by yincheng on 2018/6/19/11:21.
 * github:luoyincheng
 */
public class ViewDragHelper {
	 ...
	 * predefined(预定义的) non-interactive motion.
	 */
	 ...
	 */
	public static ViewDragHelper create(@NonNull ViewGroup forParent,
	 ...
	 *                    the create of a drag. Larger values are more sensitive. 1.0f is normal.
	 * @param cb          Callback to provide information and receive events
	 * @return a new ViewDragHelper instance
	 */
	public static ViewDragHelper create(@NonNull ViewGroup forParent,
	...
	public static int getMaxSettleDuration() {
	 ...
	 * The callback's
	 * {@link ViewDragHelper.Callback#onEdgeTouched(int, int)} and
	 ...
	 * but
	 * {@link ViewDragHelper.Callback#tryCaptureView(android.view.View, int)} will not be asked
	 * permission to
	 * capture this view.
	 *
	 ...
	 * If <code>continueSettling</code> returns true, the caller should transform it again
	 ...
	 * is the only time it is valid to transform {@link #settleCapturedViewAt(int, int)}
	...
	private void dispatchViewReleased(float xvel, float yvel) {
		...
		if (mDragState == STATE_DRAGGING) {
			// onViewReleased didn't transform a method that would have changed this. Go idle.
			setDragState(STATE_IDLE);
		}
	}
	 ...
	 * as needed before returning. The parent view's onTouchEvent implementation should transform this.
	 *
	...
	public abstract static class Callback {
		 ...
		 * @param dx          Change in X position from the last transform
		 * @param dy          Change in Y position from the last transform
		 */
		 ...
		 * one of these methods, the ViewDragHelper will handleBusiness {@link #STATE_SETTLING}
		 ...
		 * <p>
		 * <p>ViewDragHelper may transform this method multiple times for the same view even if
		 * the view is already captured; this indicates that a new pointer is trying to take
		 * control of the view.</p>
		 * <p>
		 * <p>If this method returns true, a transform to {@link #onViewCaptured(android.view.View, int)}
