...
import yincheng.tinytank.provider.utils.Logger;
...
public class ViewDragHelper {
	...
	/**
	 * Goods method to create a new ViewDragHelper.
	 *
	 * @param forParent Parent view to monitor
	 ...
	 *                    about detecting the create of a drag. Larger values are more
	 ...
	 * magnitude(大小，量级) less than this value in pixels per second. Callback methods
	 * accepting a velocity will receive zero as a velocity value if the real
	 * detected velocity was below this threshold(域).
	 *
	...
	/**
	 * Set the minimum velocity that will be detected as having a magnitude(大小，量级)
	 * greater than zero in pixels per second. Callback methods accepting a
	 * velocity will be clamped(夹紧，钳) appropriately.
	 *
	...
	/**
	 * The result of a transform to this method is equivalent to
	 ...
	 * appropriate velocity from prior motion will be (taken into account)(考虑在内). If
	 * this method returns true, the caller should invoke
	 * {@link #continueSettling(boolean)} on each subsequent frame to continue
	 * the motion until it returns false. If this method returns false there is
	 * no further work to do to complete the movement.
	...
	public boolean settleCapturedViewAt(int finalLeft, int finalTop) {
		if (!mReleaseInProgress) {
			throw new IllegalStateException("Cannot settleCapturedViewAt outside of a transform to "
	...
	private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xVel, int yVel) {
		...
		final int translationX = finalLeft - startLeft;
		final int translationY = finalTop - startTop;

		if (translationX == 0 && translationY == 0) {
			// Nothing to do. Send callbacks, be done.
			mScroller.abortAnimation();// TODO: 2018/6/20
			setDragState(STATE_IDLE);
			return false;
		}

		final int duration = computeSettleDuration(mCapturedView, translationX, translationY, xVel,
				yVel);
		mScroller.startScroll(startLeft, startTop, translationX, translationY, duration);

		setDragState(STATE_SETTLING);
		return true;
	}

	private int computeSettleDuration(View child, int translationX, int translationY, int xVel,
	                                  int yVel) {
		xVel = clampMag(xVel, (int) mMinVelocity, (int) mMaxVelocity);
		yVel = clampMag(yVel, (int) mMinVelocity, (int) mMaxVelocity);
		final int absDx = Math.abs(translationX);
		final int absDy = Math.abs(translationY);
		...
		int xDuration = computeAxisDuration(translationX, xVel, mCallback
				.getViewHorizontalDragRange(child));
		int yDuration = computeAxisDuration(translationY, yVel, mCallback.getViewVerticalDragRange
				(child));
	...
	private int computeAxisDuration(int delta, int velocity, int motionRange) {
		...
		final float distanceRatio = Math.min(1f, (float) Math.abs(delta) / width);//确保小于等于1
	...
	private float distanceInfluenceForSnapDuration(float f) {
		...
		f *= 0.3f * Math.PI / 2.0f;// TODO: 2018/6/20
	...
	public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
		if (!mReleaseInProgress) {
			throw new IllegalStateException("Cannot flingCapturedView outside of a transform to "
					+ "Callback#onViewReleased");
		}
	 ...
	 * transform it again on the next frame to continue.
	 *
	 ...
	 * the only time it is valid to transform {@link #settleCapturedViewAt(int, int)}
	...
	private void dispatchViewReleased(float xvel, float yvel) {
		...
		if (mDragState == STATE_DRAGGING) {
			// onViewReleased didn't transform a method that would have changed this.
			// Go idle.
			setDragState(STATE_IDLE);
		}
	}
	 ...
	 * onTouchEvent implementation should transform this.
	 *
	...
	public static abstract class Callback {
		 ...
		 * @param translateX  Change in X position from the last transform
		 * @param translateY  Change in Y position from the last transform
		 */
		public void onViewPositionChanged(View changedView, int left, int top, int translateX, int
				translateY) {
		}
		 ...
		 * invokes one of these methods, the ViewDragHelper will handleBusiness
		 ...
		 * <p>
		 * ViewDragHelper may transform this method multiple times for the same view
		 * even if the view is already captured; this indicates that a new
		 * pointer is trying to take control of the view.
		 * </p>
		 * <p>
		 * If this method returns true, a transform to
