...
public class SubSamplingScaleImageView extends AppCompatImageView {
	...
	private PointF leftTopRelativePoint;
	private PointF leftTopRelativePointStart;
	private PointF leftTopRelativePointStartBefore;
	...
	public SubSamplingScaleImageView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		if (attrs != null) {
			TypedArray typedAttr = getContext().obtainStyledAttributes(attrs, R.styleable.SubSamplingScaleImageView);
	 ...
	 * initialised after the transform is made. This is a hint only; the bundled {@link ImageDecoder} and
	...
	public final void setImage(@NonNull ImageSource imageSource, ImageSource previewSource, ImageViewState state) {
		...
		if (previewSource != null) {
			...
			this.sourceWidth = imageSource.getSWidth();
			this.sourceHeight = imageSource.getSHeight();
	...
	private void reset(boolean newImage) {
		...
		scale = 0f;
		scaleStart = 0f;
		leftTopRelativePoint = null;
		leftTopRelativePointStart = null;
		leftTopRelativePointStartBefore = null;
		pendingScale = 0f;
		sPendingCenter = null;
	...
	private void setGestureDetector(final Context context) {
		Log.i("subsampling", "初始化etector");
		this.detector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {

			@Override
			public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
				if (panEnabled && readySent && leftTopRelativePoint != null && e1 != null && e2 != null && (Math.abs(e1.getX() - e2.getX()) > 50 || Math.abs(e1.getY() - e2.getY()) > 50) && (Math.abs(velocityX) > 500 || Math.abs(velocityY) > 500) && !isZooming) {
					PointF vTranslateEnd = new PointF(leftTopRelativePoint.x + (velocityX * 0.25f), leftTopRelativePoint.y + (velocityY * 0.25f));
			...
			public boolean onSingleTapConfirmed(MotionEvent e) {
				Log.i("subsampling", "detector:onSingleTapConfirmed");
//                onClicked();
				performClick();
				return true;
			}
			...
			public boolean onDoubleTap(MotionEvent e) {
				if (zoomEnabled && readySent && leftTopRelativePoint != null) {
					...
					if (quickScaleEnabled) {
						...
						leftTopRelativePointStart = new PointF(leftTopRelativePoint.x, leftTopRelativePoint.y);
						scaleStart = scale;
						isQuickScaling = true;
						isZooming = true;
		...
		Log.i("subsampling", "初始化singleDetector");
		singleDetector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
			@Override
			public boolean onSingleTapConfirmed(MotionEvent e) {
				Log.i("subsampling", "singleDetector:onSingleTapConfirmed");
				performClick();
				return true;
			}
		});
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (sourceWidth > 0 && sourceHeight > 0) {
			if (resizeWidth && resizeHeight) {
				width = getSourceWidthConsideredRotate();
				height = getSourceHeightConsideredRotate();
			} else if (resizeHeight) {
				height = (int) ((((double) getSourceHeightConsideredRotate() / (double) getSourceWidthConsideredRotate()) * width));
			} else if (resizeWidth) {
				width = (int) ((((double) getSourceWidthConsideredRotate() / (double) getSourceHeightConsideredRotate()) * height));
			}
		}
	...
	public boolean onTouchEvent(@NonNull MotionEvent event) {
		...
		// Abort if not ready
		if (leftTopRelativePoint == null) {
			Log.i("subsampling", "leftTopRelativePoint == null");
			if (singleDetector != null) {
				Log.i("subsampling", "singleDetector != null");
		...
		if (leftTopRelativePointStart == null) {
			leftTopRelativePointStart = new PointF(0, 0);
		}
		if (leftTopRelativePointStartBefore == null) {
			leftTopRelativePointStartBefore = new PointF(0, 0);
		...
		leftTopRelativePointStartBefore.set(leftTopRelativePoint);
		boolean handled = onTouchEventInternal(event);
		sendStateChanged(scaleBefore, leftTopRelativePointStartBefore, ORIGIN_TOUCH);
		Log.i("subsampling", "leftTopRelativePoint:" + leftTopRelativePoint.toString());
	...
	private boolean onTouchEventInternal(@NonNull MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_POINTER_2_DOWN:
				...
				if (touchCount >= 2) {
					Log.i("subsampling", "double finger");
					if (zoomEnabled) {
						// Start pinch to zoom. Calculate distance between touch points and center point of the pinch.
						float distance = distance(event.getX(0), event.getX(1), event.getY(0), event.getY(1));
						scaleStart = scale;
						vDistStart = distance;
						leftTopRelativePointStart.set(leftTopRelativePoint.x, leftTopRelativePoint.y);
...
//                    if (!isEnabledOperation) {//是否为选中状态
//                        Log.i("subsampling", "one finger tap");
//                        return false;
//                    }
					leftTopRelativePointStart.set(leftTopRelativePoint.x, leftTopRelativePoint.y);
				...
				if (maxTouchCount > 0) {
					if (touchCount >= 2) {
						...
						if (zoomEnabled && (distance(vCenterStart.x, vCenterEndX, vCenterStart.y, vCenterEndY) > 5 || Math.abs(vDistEnd - vDistStart) > 5 || isPanning)) {
							...
							Log.i("subsampling", "action_innermove:"
									+ "[previousScale:" + previousScale + "]--"
									+ "[scale:" + scale + "]--"
									+ "[vDistEnd:" + vDistEnd + "]--"
									+ "[vDistStart:" + vDistStart + "]--"
							...
							if (scale <= minScale()) {
								...
								leftTopRelativePointStart.set(leftTopRelativePoint);
							} else if (panEnabled) {
								// Translate to place the source image coordinate that was at the center of the pinch at the start
								// at the center of the pinch now, to give simultaneous pan + zoom.
								float vLeftStart = vCenterStart.x - leftTopRelativePointStart.x;
								float vTopStart = vCenterStart.y - leftTopRelativePointStart.y;
								float vLeftNow = vLeftStart * (scale / scaleStart);
								float vTopNow = vTopStart * (scale / scaleStart);
								leftTopRelativePoint.x = vCenterEndX - vLeftNow;
								leftTopRelativePoint.y = vCenterEndY - vTopNow;
								if ((previousScale * getSourceHeightConsideredRotate() < getHeight() && scale * getSourceHeightConsideredRotate() >= getHeight()) || (previousScale * getSourceWidthConsideredRotate() < getWidth() && scale * getSourceWidthConsideredRotate() >= getWidth())) {
									fitToBounds(true);
									vCenterStart.set(vCenterEndX, vCenterEndY);
									leftTopRelativePointStart.set(leftTopRelativePoint);
									scaleStart = scale;
								...
								leftTopRelativePoint.x = (getWidth() / 2) - (scale * sRequestedCenter.x);
								leftTopRelativePoint.y = (getHeight() / 2) - (scale * sRequestedCenter.y);
							} else {
								// With no requested center, scale around the image center.
								leftTopRelativePoint.x = (getWidth() / 2) - (scale * (getSourceWidthConsideredRotate() / 2));
								leftTopRelativePoint.y = (getHeight() / 2) - (scale * (getSourceHeightConsideredRotate() / 2));
							}
						...
						Log.i("subsampling", "action_innermove:isQuickScaling");
						// One finger zoom
						...
						if (spanDiff > 0.03f || quickScaleMoved) {
							...
							if (panEnabled) {
								...
								float vLeftNow = vLeftStart * (scale / scaleStart);
								float vTopNow = vTopStart * (scale / scaleStart);
								leftTopRelativePoint.x = vCenterStart.x - vLeftNow;
								leftTopRelativePoint.y = vCenterStart.y - vTopNow;
								if ((previousScale * getSourceHeightConsideredRotate() < getHeight() && scale * getSourceHeightConsideredRotate() >= getHeight()) || (previousScale * getSourceWidthConsideredRotate() < getWidth() && scale * getSourceWidthConsideredRotate() >= getWidth())) {
									fitToBounds(true);
						...
						Log.i("subsampling", "action_innermove:isZooming");
						...
						if (dx > offset || dy > offset || isPanning) {
							consumed = true;
							leftTopRelativePoint.x = leftTopRelativePointStart.x + (event.getX() - vCenterStart.x);
							leftTopRelativePoint.y = leftTopRelativePointStart.y + (event.getY() - vCenterStart.y);

							float lastX = leftTopRelativePoint.x;
							float lastY = leftTopRelativePoint.y;
							fitToBounds(true);
							boolean atXEdge = lastX != leftTopRelativePoint.x;
							boolean atYEdge = lastY != leftTopRelativePoint.y;
							boolean edgeXSwipe = atXEdge && dx > dy && !isPanning;
							boolean edgeYSwipe = atYEdge && dy > dx && !isPanning;
							boolean yPan = lastY == leftTopRelativePoint.y && dy > offset * 3;
							...
							if (!panEnabled) {
								leftTopRelativePoint.x = leftTopRelativePointStart.x;
								leftTopRelativePoint.y = leftTopRelativePointStart.y;
								requestDisallowInterceptTouchEvent(false);
							}
			...
			case MotionEvent.ACTION_POINTER_2_UP:
				...
				if (isQuickScaling) {
					isQuickScaling = false;
					if (!quickScaleMoved) {
						Log.i("subsampling", "action_inner_up:quickScaleMoved");
						doubleTapZoom(quickScaleSCenter, vCenterStart);
					}
				}
				if (maxTouchCount > 0 && (isZooming || isPanning)) {
					Log.i("subsampling", "action_inner_up: zooming or  panning");
					...
					Log.i("subsampling", "action_inner_up:not zooming && not panning");
				}
				if (touchCount == 1) {
					isZooming = false;
					isPanning = false;
					maxTouchCount = 0;
					Log.i("subsampling", "action_inner_up");
	...
	private void doubleTapZoom(PointF sCenter, PointF vFocus) {
		if (!panEnabled) {
			if (sRequestedCenter != null) {
				...
				// With no requested center, scale around the image center.
				sCenter.x = getSourceWidthConsideredRotate() / 2;
				sCenter.y = getSourceHeightConsideredRotate() / 2;
			}
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		// dimensions and therefore the first opportunity to set scale and translate. If this transform returns
		...
		if (anim != null && anim.vFocusStart != null) {
			...
			leftTopRelativePoint.x -= sourceToViewX(anim.sCenterEnd.x) - vFocusNowX;
			leftTopRelativePoint.y -= sourceToViewY(anim.sCenterEnd.y) - vFocusNowY;

			// For translate anims, showing the image non-centered is never allowed, for scaling anims it is during the animation.
			fitToBounds(finished || (anim.scaleStart == anim.scaleEnd));
			sendStateChanged(scaleBefore, leftTopRelativePointStartBefore, anim.origin);
		...
		if (tileMap != null && isBaseLayerReady()) {
			...
			if (bitmapIsPreview) {
				xScale = scale * ((float) sourceWidth / bitmap.getWidth());
				yScale = scale * ((float) sourceHeight / bitmap.getHeight());
			}
			...
			matrix.postTranslate(leftTopRelativePoint.x, leftTopRelativePoint.y);
			...
			if (tileBgPaint != null) {
				...
				sRect.set(0f, 0f, bitmapIsPreview ? bitmap.getWidth() : sourceWidth, bitmapIsPreview ? bitmap.getHeight() : sourceHeight);
		...
		if (debug) {
			...
			canvas.drawText("Translate: " + String.format(Locale.ENGLISH, "%.2f", leftTopRelativePoint.x) + ":" + String.format(Locale.ENGLISH, "%.2f", leftTopRelativePoint.y), px(5), px(30), debugTextPaint);
	...
	private boolean checkReady() {
		boolean ready = getWidth() > 0 && getHeight() > 0 && sourceWidth > 0 && sourceHeight > 0 && (bitmap != null || isBaseLayerReady());
		if (!readySent && ready) {
			prepareForNextDraw();
			readySent = true;
			onReady();
	...
	private synchronized void initialiseBaseLayer(@NonNull Point maxTileDimensions) {
		...
		if (fullImageSampleSize == 1 && sRegion == null && getSourceWidthConsideredRotate() < maxTileDimensions.x && getSourceHeightConsideredRotate() < maxTileDimensions.y) {
	 ...
	 */
	private void prepareForNextDraw() {
		if (getWidth() == 0 || getHeight() == 0 || sourceWidth <= 0 || sourceHeight <= 0) {
			return;
		}
		...
		if (sPendingCenter != null && pendingScale != null) {
			scale = pendingScale;
			if (leftTopRelativePoint == null) {
				leftTopRelativePoint = new PointF();
			}
			leftTopRelativePoint.x = (getWidth() / 2) - (scale * sPendingCenter.x);
			leftTopRelativePoint.y = (getHeight() / 2) - (scale * sPendingCenter.y);
			sPendingCenter = null;
			pendingScale = null;
			fitToBounds(true);
			refreshRequiredTiles(true);
	...
	private int calculateInSampleSize(float scale) {
		...
		int reqWidth = (int) (getSourceWidthConsideredRotate() * scale);
		int reqHeight = (int) (getSourceHeightConsideredRotate() * scale);
		...
		if (getSourceHeightConsideredRotate() > reqHeight || getSourceWidthConsideredRotate() > reqWidth) {

			// Calculate ratios of height and width to requested height and width
			final int heightRatio = Math.round((float) getSourceHeightConsideredRotate() / (float) reqHeight);
			final int widthRatio = Math.round((float) getSourceWidthConsideredRotate() / (float) reqWidth);
	...
	private void fitToBounds(boolean center, ScaleAndTranslate sat) {
		...
		float scaleWidth = scale * getSourceWidthConsideredRotate();
		float scaleHeight = scale * getSourceHeightConsideredRotate();
	...
	private void fitToBounds(boolean center) {
		...
		if (leftTopRelativePoint == null) {
			init = true;
			leftTopRelativePoint = new PointF(0, 0);
		}
		if (satTemp == null) {
			satTemp = new ScaleAndTranslate(0, new PointF(0, 0));
		}
		satTemp.scale = scale;
		satTemp.vTranslate.set(leftTopRelativePoint);
		fitToBounds(center, satTemp);
		scale = satTemp.scale;
		leftTopRelativePoint.set(satTemp.vTranslate);
		if (init && minimumScaleType != SCALE_TYPE_START) {
			leftTopRelativePoint.set(vTranslateForSCenter((float) getSourceWidthConsideredRotate() / 2, (float) getSourceHeightConsideredRotate() / 2, scale));
		}
	}
			...
			for (int x = 0; x < xTiles; x++) {
				for (int y = 0; y < yTiles; y++) {
					...
					tile.sRect = new Rect(
							x * sTileWidth,
							y * sTileHeight,
							x == xTiles - 1 ? getSourceWidthConsideredRotate() : (x + 1) * sTileWidth,
							y == yTiles - 1 ? getSourceHeightConsideredRotate() : (y + 1) * sTileHeight
					);
	...
	private synchronized void onTilesInited(ImageRegionDecoder decoder, int sWidth, int sHeight, int sOrientation) {
		debug("onTilesInited sourceWidth=%d, sourceHeight=%d, sourceOrientation=%d", sWidth, sHeight, orientation);
		// If actual dimensions don't match the declared size, reset everything.
		if (this.sourceWidth > 0 && this.sourceHeight > 0 && (this.sourceWidth != sWidth || this.sourceHeight != sHeight)) {
			reset(false);
			if (bitmap != null) {
				if (!bitmapIsCached) {
					bitmap.recycle();
	...
	private synchronized void onImageLoaded(Bitmap bitmap, int sOrientation, boolean bitmapIsCached) {
		debug("onImageLoaded");
		// If actual dimensions don't match the declared size, reset everything.
		if (this.sourceWidth > 0 && this.sourceHeight > 0 && (this.sourceWidth != bitmap.getWidth() || this.sourceHeight != bitmap.getHeight())) {
			reset(false);
		}
	...
	private int getSourceWidthConsideredRotate() {
	 ...
	 */
	@SuppressWarnings("SuspiciousNameCombination")
	private int getSourceHeightConsideredRotate() {
		int rotation = getRequiredRotation();
		if (rotation == 90 || rotation == 270) {
			return sourceWidth;
		} else {
	...
	private float viewToSourceX(float vx) {
		...
		return (vx - leftTopRelativePoint.x) / scale;
	}
	...
	private float viewToSourceY(float vy) {
		...
		return (vy - leftTopRelativePoint.y) / scale;
	}
	...
	public void viewToFileRect(Rect vRect, Rect fRect) {
		if (leftTopRelativePoint == null || !readySent) {
			return;
		}
		fRect.set(
		...
		fRect.set(
				...
				Math.min(sourceWidth, fRect.right),
				Math.min(sourceHeight, fRect.bottom)
		);
		if (sRegion != null) {
	...
	private float sourceToViewX(float sx) {
		...
		return (sx * scale) + leftTopRelativePoint.x;
	}
	...
	private float sourceToViewY(float sy) {
		...
		return (sy * scale) + leftTopRelativePoint.y;
	}
	...
	private float minScale() {
		...
		if (minimumScaleType == SCALE_TYPE_CENTER_CROP || minimumScaleType == SCALE_TYPE_START) {
			return Math.max((getWidth() - hPadding) / (float) getSourceWidthConsideredRotate(), (getHeight() - vPadding) / (float) getSourceHeightConsideredRotate());
		} else if (minimumScaleType == SCALE_TYPE_CUSTOM && minScale > 0) {
			return minScale;
		} else {
			return Math.min((getWidth() - hPadding) / (float) getSourceWidthConsideredRotate(), (getHeight() - vPadding) / (float) getSourceHeightConsideredRotate());
		}
	}
	...
	public final void getPanRemaining(RectF vTarget) {
		...
		if (panLimit == PAN_LIMIT_CENTER) {
			vTarget.top = Math.max(0, -(leftTopRelativePoint.y - (getHeight() / 2)));
			vTarget.left = Math.max(0, -(leftTopRelativePoint.x - (getWidth() / 2)));
			vTarget.bottom = Math.max(0, leftTopRelativePoint.y - ((getHeight() / 2) - scaleHeight));
			vTarget.right = Math.max(0, leftTopRelativePoint.x - ((getWidth() / 2) - scaleWidth));
		} else if (panLimit == PAN_LIMIT_OUTSIDE) {
			vTarget.top = Math.max(0, -(leftTopRelativePoint.y - getHeight()));
			vTarget.left = Math.max(0, -(leftTopRelativePoint.x - getWidth()));
			vTarget.bottom = Math.max(0, leftTopRelativePoint.y + scaleHeight);
			vTarget.right = Math.max(0, leftTopRelativePoint.x + scaleWidth);
		} else {
			vTarget.top = Math.max(0, -leftTopRelativePoint.y);
			vTarget.left = Math.max(0, -leftTopRelativePoint.x);
			vTarget.bottom = Math.max(0, (scaleHeight + leftTopRelativePoint.y) - getHeight());
			vTarget.right = Math.max(0, (scaleWidth + leftTopRelativePoint.x) - getWidth());
		}
	}
	...
	public final void resetScaleAndCenter() {
		...
		if (isReady()) {
			this.sPendingCenter = new PointF(getSourceWidthConsideredRotate() / 2, getSourceHeightConsideredRotate() / 2);
		} else {
	...
	/**
	 * Sets the image orientation. It's best to transform this before setting the image file or asset, because it may waste
	...
	public final ImageViewState getState() {
		if (leftTopRelativePoint != null && sourceWidth > 0 && sourceHeight > 0) {
			//noinspection ConstantConditions
	...
	public final void setPanEnabled(boolean panEnabled) {
		...
		if (!panEnabled && leftTopRelativePoint != null) {
			leftTopRelativePoint.x = (getWidth() / 2) - (scale * (getSourceWidthConsideredRotate() / 2));
			leftTopRelativePoint.y = (getHeight() / 2) - (scale * (getSourceHeightConsideredRotate() / 2));
			if (isReady()) {
				refreshRequiredTiles(true);
	...
	private void sendStateChanged(float oldScale, PointF oldVTranslate, int origin) {
		if (onStateChangedListener != null && scale != oldScale) {
			...
			Log.i("subsampling", "onScaleChanged()");
		}
		if (onStateChangedListener != null && !leftTopRelativePoint.equals(oldVTranslate)) {
			onStateChangedListener.onCenterChanged(getCenter(), origin);
			Log.i("subsampling", "onCenterChanged()");
		}
	}
	 ...
	 * then set your options and transform {@link #start()}.
	 */
